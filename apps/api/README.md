...
14. Artifact retention P0 记录 `retention_policy` 和可选 `retain_until`；内部 service sweep 可删除过期且未被 model_run evidence refs 引用的 `ttl` artifact，并对被引用 artifact 做保护。当前仍没有公开 admin API、调度器、archive backend 或 UI。
