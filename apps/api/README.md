...
14. Artifact retention P0 记录 `retention_policy` 和可选 `retain_until`；`POST /api/v1/admin/artifacts/retention-sweep` 需要 `artifact:admin` scope，默认 `dry_run=true`，可手动删除过期且未被 model_run evidence refs 引用的 `ttl` artifact，并对被引用 artifact 做保护。当前仍没有调度器、archive backend 或 UI。
