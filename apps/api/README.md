...
Phase 4A 仍使用单个 `internal/compute` package 收敛 skeleton，后续领域稳定后再拆分 `internal/domain/*`。当前 metadata `Store` 已先由 12 个领域子接口组合，job create/list/cancel/complete/fail/timeout 已委派到 `JobLifecycleService`；artifact upload / listing / metadata lookup / download / retention / archive 逻辑已委派到 `ArtifactLifecycleService`；worker register / claim / heartbeat 已委派到 `WorkerLifecycleService`；simulation input / process graph 注册与 `simulation_request.input_ref` 解析已委派到只依赖 simulation input、process graph、model run、job 读取接口的 `SimulationInputService`；draft confirmation / promotion 已委派到 `DraftWorkflowService`；result explanation submit/review/publish 已委派到 `ResultExplanationService`；model catalog / benchmark run / model run governance 查询与 benchmark case queueing 已委派到 `ModelGovernanceService`；result read / evidence package / production readiness / evidence-ref resolution 已委派到 `EvidenceGovernanceService`；metrics snapshot 已委派到 `MetricsService`。主 `Service` 主要保留 package-level wiring 与兼容委派。使用 `scripts/audit-compute-api-boundary.ps1` 验证 Store/domain 子接口、调用来源与实现覆盖。
...
5. P0 auth 使用静态 Bearer token + scope，并支持配置级 `revoked` token 拒绝；token config 可选 `tenant_id` / `project_id`，用于 job list/get 和 artifact download 的 HTTP read-scope 过滤；空 tenant/project 表示全局读。当前不实现完整 RBAC 或动态 token 管理 API。`cmd/compute-api` 在 `APP_ENV=production` 或 `ENVIRONMENT=production` 时必须拒绝空 `COMPUTE_API_TOKENS_JSON` 和默认 dev token（`dev-public-token`、`dev-worker-token`、`dev-admin-token`）。Result explanation review/publish 使用 `explanation:write`，避免复用 job 创建权限表达解释发布。当前仅 job create/queue 与 artifact retention delete/archive events 写入 selected mutation audit envelope；完整 all-mutation audit、OIDC/RBAC、site scope 和全对象 tenant/project/site data scope 仍是后续安全工作。
