...
11. `POST /api/v1/contracts/validate` 只校验已有 `contracts/` schema（当前包括 `simulation_request.v1`、`agent_scenario_draft.v1`、`constraint_draft.v1`、`draft_confirmation.v1`、`result_explanation.v1`、`model_catalog.v1` 等）；未知 future schema 必须返回 invalid，不得伪造支持。`POST /api/v1/contracts/confirm-draft` 会校验确认记录和嵌入草案，并在首次新建时持久化 compact `draft_confirmation.recorded` audit event；重复同 hash 幂等请求不重复写 audit；它仍不创建 job、不执行审批或生产动作。`GET /api/v1/contracts/confirmations/{confirmation_id}` 用于只读审计查询，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。`GET /api/v1/contracts/confirmations/{confirmation_id}/constraint-application-plan` 只对 approved `constraint_draft.v1` 生效，返回 advisory plan，且必须保持 `would_create_job=false` / `would_modify_target=false`；该 read path 同样按 confirmation scope 拒绝；稳定 plan shape 与 warnings 位于 `internal/domain/agent`。`POST /api/v1/contracts/confirmations/{confirmation_id}/promote-simulation-check` 只对 approved `agent_scenario_draft.v1` 生效，且只在嵌入 `proposed_request` 本身是完整 `simulation_request.v1` 时创建 simulation check job；promotion path 会先按 confirmation scope 拒绝；stable proposed request extraction 位于 `internal/domain/agent`，schema validation 和 job 创建仍留在 compute。
