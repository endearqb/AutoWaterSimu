...
- model catalog snapshot registration/read/list query with tenant/project/site scoped reads for persisted snapshots, scoped registration/status mutation checks, default parameter set status transition, default parameter set promotion planning, evidence-backed default parameter set approval promotion, benchmark case job scheduling, and model run persistence/list/search/read query。
...
- evidence governance summary for model/parameter production-readiness review, using the source job tenant/project/site to select scoped persisted model catalog snapshots and constrain stored process graph evidence-ref dereference。
...
Draft confirmation / result explanation metadata persistence 仍由 `internal/compute` 的 `MemoryStore` 与 `PostgresStore` 实现；原宽泛 `memory_agent.go` 与 `postgres_agent.go` 已按 `DraftConfirmationStore` 和 `ResultExplanationStore` 责任拆分为 `memory_draft_confirmations.go`、`memory_result_explanations.go`、`postgres_draft_confirmations.go` 和 `postgres_result_explanations.go`，draft confirmation 持久化包含 optional `site_id` 与 compact selected mutation audit，result explanation review/publish 状态流与 clone 语义保持不变。

`EvidenceGovernanceService` 仍留在 `internal/compute` compatibility package；`evidence_governance.go` 仅保留 struct/constructor/store/callback wiring，result read、evidence package export、production readiness 和 evidence-ref resolution workflow 分组位于同 package 的 `evidence_governance_*.go` 文件；evidence package / production-readiness governance 会把 source job tenant/project/site 传给 model catalog read selection。
...
`DraftWorkflowService` 仍留在 `internal/compute` compatibility package；`draft_workflows.go` 仅保留 struct/constructor/callback wiring，draft confirmation validation/persistence/read、compact mutation audit、constraint application plan 和 explicit simulation-check promotion workflow 分组位于同 package 的 `draft_workflows_*.go` 文件。
...
5. P0 auth 使用静态 Bearer token + scope，并支持配置级 `revoked` token 拒绝；token config 可选 `tenant_id` / `project_id` / `site_id`，用于 job list/get、process_graph get、simulation_input get、draft confirmation get / constraint plan / promotion、persisted model catalog root/model/snapshot reads、model_run get / job-filtered list、benchmark_run get / job-filtered list 和 artifact download 的 HTTP read-scope 过滤，也用于 evidence package / production-readiness governance 的 model catalog selection，以及 `POST /api/v1/compute/jobs`、`POST /api/v1/compute/jobs/{job_id}/cancel`、artifact retention sweep candidate filtering、worker claim candidate filtering、worker heartbeat/artifact/result completion job mutation、`POST /api/v1/contracts/confirm-draft`、`POST /api/v1/compute/jobs/{job_id}/result-explanations` 及其 review/publish 子路径、`POST /api/v1/simulation-checks`、`POST /api/v1/contracts/confirmations/{confirmation_id}/promote-simulation-check`、`POST /api/v1/model-catalog/{model_key}/versions/{model_version}/benchmark-cases/{benchmark_case_id}/schedule-run`、`POST /api/v1/model-catalog`、default parameter set status/promote-approved mutation、benchmark_run registration、`POST /api/v1/process-graphs` 和 `POST /api/v1/simulation-inputs` 的 tenant/project/site 写入边界检查；scoped token 访问 `GET /api/v1/model-runs`、`GET /api/v1/model-catalog/{model_key}/versions/{model_version}/benchmark-runs`、model catalog promotion plan 或 promote-approved 必须提供已授权的 `job_id`，避免跨 job 枚举和跨 job promotion evidence。空 tenant/project/site 表示全局读写边界。当前不实现完整 RBAC 或动态 token 管理 API。`cmd/compute-api` 在 `APP_ENV=production` 或 `ENVIRONMENT=production` 时必须拒绝空 token config 和默认 dev token（`dev-public-token`、`dev-worker-token`、`dev-admin-token`）；生产 token config 可通过 `COMPUTE_API_TOKENS_JSON` 或挂载 secret 文件 `COMPUTE_API_TOKENS_FILE` 提供，二者不得同时设置。Result explanation submit/review/publish 使用 `explanation:write`，避免复用 job 创建权限表达解释发布。当前 job create/queue、job cancel/timeout、worker claim/heartbeat lease refresh/artifact upload/result completion、draft promotion / benchmark schedule-run job create/queue、artifact retention delete/archive 与 result explanation submit/review/publish events 写入 `compute_job_events.event_json.audit`；worker registration、draft_confirmation record、process_graph registration、simulation_input registration、model catalog registration、default parameter set status/promote 和 benchmark_run registration 写入 compact `mutation_audit_events`。完整 all-mutation audit、OIDC/RBAC、剩余 mutation data-scope 和全对象 tenant/project/site data scope 仍是后续安全工作。
...
11. `POST /api/v1/contracts/validate` 只校验已有 `contracts/` schema（当前包括 `simulation_request.v1`、`agent_scenario_draft.v1`、`constraint_draft.v1`、`draft_confirmation.v1`、`result_explanation.v1`、`model_catalog.v1` 等）；未知 future schema 必须返回 invalid，不得伪造支持。`POST /api/v1/contracts/confirm-draft` 会校验确认记录和嵌入草案，按 payload metadata tenant/project/site 在写入前拒绝跨 scope scoped token，并在首次新建时持久化 compact `draft_confirmation.recorded` audit event；重复同 hash 幂等请求不重复写 audit；它仍不创建 job、不执行审批或生产动作。`GET /api/v1/contracts/confirmations/{confirmation_id}` 用于只读审计查询，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。`GET /api/v1/contracts/confirmations/{confirmation_id}/constraint-application-plan` 只对 approved `constraint_draft.v1` 生效，返回 advisory plan，且必须保持 `would_create_job=false` / `would_modify_target=false`；该 read path 同样按 confirmation scope 拒绝；稳定 plan shape 与 warnings 位于 `internal/domain/agent`。`POST /api/v1/contracts/confirmations/{confirmation_id}/promote-simulation-check` 只对 approved `agent_scenario_draft.v1` 生效，且只在嵌入 `proposed_request` 本身是完整 `simulation_request.v1` 时创建 simulation check job；promotion path 会先按 confirmation scope 拒绝，再按 proposed `simulation_request` scope 约束 input-ref resolution 和最终 job create，跨 scope proposed request 不会写入 job、registry record 或 audit event；stable proposed request extraction 位于 `internal/domain/agent`，schema validation 和 job 创建仍留在 compute。
12. `POST /api/v1/model-catalog` 可登记 schema-valid `model_catalog.v1` 快照，使用 `catalog_id`（默认 `default`）+ payload hash 幂等；scoped token 写入前会按 payload `metadata.tenant_id` / `project_id` / `site_id` 校验 data-scope，不匹配时拒绝且不落库。`GET /api/v1/model-catalog` / `GET /api/v1/model-catalog/{model_key}` 优先读取最新持久化 `default` catalog，未登记时回退到 built-in catalog（material_balance 含 approved default parameter set；ASM1Slim/ASM1/ASM3/UDM 只含 worker-smoke-backed model version 与 benchmark case，不含默认参数集）。Built-in catalog 的稳定 document shape 位于 `internal/domain/models`；material-balance default parameter hash、typed DTO conversion、schema validation、持久化 catalog fallback/read behavior 仍由 compute 负责。`GET /api/v1/model-catalog/{model_key}/versions/{model_version}/default-parameter-set/promotion-plan` 只读评估当前 default parameter set 的 latest benchmark_run / model_run 证据是否足以建议从 `validated` 提升到 `approved`，不执行 benchmark、不修改 catalog、不完成生产审批；scoped token 暂时不能调用该 plan。`POST /api/v1/model-catalog/{model_key}/versions/{model_version}/default-parameter-set/promote-approved` 需要 `model:write`，只在同一 promotion plan 通过时把当前 default parameter set 从 `validated` 晋升到 `approved` 并生成新的 catalog snapshot；scoped token 暂时不能调用该 endpoint，直到 promotion evidence filtering 可按 job/data scope 收敛；该 endpoint 仍不执行 benchmark、不记录 benchmark_run、不代表生产审批完成。`POST /api/v1/model-catalog/{model_key}/versions/{model_version}/benchmark-cases/{benchmark_case_id}/schedule-run` 只把 active model version 的 validated benchmark case 排队为标准 `compute_job.v1`，要求当前 default parameter set 存在且未 retired；scoped token 会按 referenced simulation input / process graph / model_run source job scope 和 request metadata 生成的 job context 做写入前 data-scope 拒绝；该 job document 的稳定 shape 位于 `internal/domain/models`，catalog lookup、gate/error mapping、execution profile lookup、simulation input resolution、JSON marshaling、createJob 和 HTTP mapping 仍由 compute 负责；该 endpoint 不执行 Python、不记录 benchmark_run、不修改 catalog、不完成参数集审批。`POST /api/v1/model-catalog/{model_key}/versions/{model_version}/default-parameter-set/status` 只允许对当前 catalog 中已有 `default_parameter_set.status` 做前向迁移或 retire，并生成新的 catalog snapshot；scoped token 只会读取并修改匹配 tenant/project/site 的 persisted catalog，且没有匹配 persisted catalog 时不会从 built-in/global catalog 创建快照；多参数集管理和自动化 approval workflow 仍是 Phase 6 后续。
13. `POST /api/v1/model-catalog/{model_key}/versions/{model_version}/benchmark-runs` 可记录 schema-valid `benchmark_run.v1`，要求 benchmark case 已存在且为 `validated`、parameter set 匹配当前 default parameter set、model_run 已存在且 parameter hash 匹配，并要求 evidence refs 可在该 job 内解析。scoped token 写入前会按 payload `job_id` 的关联 job tenant/project/site 校验 data-scope，不匹配时拒绝且不写入 benchmark run 或 mutation audit。该记录是执行历史，不创建或执行 benchmark job、不改变参数集状态、不代表生产审批完成。`GET` 列表支持按 `job_id`、`benchmark_case_id` 与 `parameter_set_id` 过滤，scoped token 必须提供已授权 `job_id`，promotion plan 使用同一历史记录面。
14. Artifact retention P0 记录 `retention_policy` 和可选 `retain_until`；`POST /api/v1/admin/artifacts/retention-sweep` 需要 `artifact:admin` scope，默认 `dry_run=true`，scoped admin token 只会枚举并处理匹配 token tenant/project/site 的 artifact candidates，可手动删除过期且未被 model_run evidence refs 引用的 `ttl` artifact，并对被引用 artifact 做保护。`COMPUTE_API_RETENTION_SWEEP_INTERVAL` 可显式启用同一逻辑的后台 scheduler，默认关闭且默认 dry-run；`COMPUTE_API_ARCHIVE_DIR` 显式配置后会启用 `local_fs_archive`；`COMPUTE_API_ARCHIVE_S3_ENDPOINT` / `COMPUTE_API_ARCHIVE_S3_BUCKET` / `COMPUTE_API_ARCHIVE_S3_ACCESS_KEY_ID` / `COMPUTE_API_ARCHIVE_S3_SECRET_ACCESS_KEY` 显式配置后会启用 path-style `s3_archive`。两类 archive backend 都对过期且未引用的 `archive_candidate` artifact 先复制、校验、持久化 archive metadata 和带 audit envelope 的 `artifact.archived` event，再删除 hot object，普通 artifact download 可从 archive store 回退读取；未配置时仍以 `archive_executor_not_configured` 跳过。
15. `compute_result.v1.risk_findings` 如由 worker 返回，会通过 `internal/domain/evidence` 的 stored summary helper 同步进入 stored result summary，便于 `GET /api/v1/compute/jobs/{job_id}/result` 和 NewSystem/milp 只读集成查看；job lifecycle 仍负责 schema validation、summary persistence 和 result hash。`GET /api/v1/compute/jobs/{job_id}/evidence-ref?ref=...` 可在 job 边界内解析 `model_run:<id>`、`artifact:<id>`、`job:<id>`、`simulation_input:<id>`、`process_graph:<id>` 和 `evidence_package:<id>`；`process_graph:<id>` 解析会拒绝存储记录 tenant/project/site 与源 job 不匹配的对象，embedded `simulation_input:<id>` payload 也会拒绝显式 metadata scope 与源 job 不匹配的对象，空 scope 继续作为 legacy/global 兼容；当前不做风险审批或生产发布动作。
16. Evidence package 会基于 source job tenant/project/site 匹配的最新持久化 model catalog（无匹配持久化时回退 built-in catalog；无 job scope 时使用 unscoped latest persisted / built-in fallback）和 persisted model runs 生成 `governance.production_allowed` 与 model version refs；该判断只用于 evidence/read 集成，不阻止 job 创建，也不执行生产审批。`governance.production_allowed` 的稳定 active+approved predicate 委派给 `internal/domain/models`，catalog lookup、evidence package assembly 和 response mapping 仍留在 compute。`GET /api/v1/compute/jobs/{job_id}/production-readiness` 在 evidence/read 边界内返回 `production_readiness.v1`，只汇总 job 成功、evidence 可用、governance 允许和 high/critical risk finding 阻断状态；它仍要求外部审批且禁止 AutoWaterSimu 自动发布生产动作。
17. `POST /api/v1/process-graphs` 可登记 schema-valid `process_graph.v1`，并按 `(process_graph_id, version)` + payload hash 做幂等；scoped token 写入前会按 payload `metadata.tenant_id` / `project_id` / `site_id` 校验 data-scope，不匹配时拒绝且不落库、不写 mutation audit；首次登记会写 compact `process_graph.registered` audit event，重复同 hash 幂等请求不重复写 audit；`GET /api/v1/process-graphs/{process_graph_id}?version=1` 返回登记记录和原始 payload，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。省略 `version` 时默认读取 version 1。稳定 process graph record data projection 位于 `internal/domain/simulation`；schema validation、compute DTO 映射、store persistence、idempotency 和 HTTP mapping 仍留在 compute。
18. `POST /api/v1/simulation-inputs` 可登记 schema-valid `simulation_input.v1`，并按 `simulation_input_id` + payload hash 做幂等；scoped token 写入前会按 payload `metadata.tenant_id` / `project_id` / `site_id` 校验 data-scope，不匹配时拒绝且不落库、不写 mutation audit；首次登记会写 compact `simulation_input.registered` audit event，重复同 hash 幂等请求不重复写 audit；`GET /api/v1/simulation-inputs/{simulation_input_id}` 返回登记记录和原始 payload，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。稳定 simulation input record data projection 位于 `internal/domain/simulation`；schema validation、compute DTO 映射、store persistence、idempotency 和 HTTP mapping 仍留在 compute。
19. `POST /api/v1/simulation-checks` 将 `simulation_request.v1` 转换为 `compute_job.v1` 并入队；direct HTTP simulation-check create 会先按 `simulation_request.metadata.tenant_id/project_id` 与 `metadata.site_id`（或 `external_refs.site_id`）执行 scoped-token mutation data-scope 拒绝，再解析 `input_ref` 与创建 job，避免跨 scope 请求先落库 registry record。当前支持 material_balance、ASM1Slim、ASM1、ASM3、UDM job types 的 `input_ref.simulation_input` 内嵌 payload、先登记后通过 `input_ref.simulation_input_id` 引用，以及通过已持久化 `input_ref.model_run_id` 找回源 job 的原始 `simulation_input.v1` payload 进行保守 replay。`input_ref.simulation_input` 缺少 metadata scope 时会继承已授权的 simulation request scope 后再自动登记；显式 metadata 与 token scope 不匹配时拒绝且不写 job、registry record 或 mutation audit。`input_ref.process_graph_id` / `process_graph_version` 自动解析仍只生成 material-balance `simulation_input.v1`，不得静默扩展到 ASM/UDM。simulation-check 路径中新建的 simulation_input registry record 使用同一 compact registration audit，并从 HTTP audit context 继承 principal/route。
20. `POST /api/v1/compute/jobs/{job_id}/result-explanations` 接收外部 Agent 生成的 `result_explanation.v1`，要求 job 已有结果、payload `job_id` 与路径一致、所有 `evidence_refs` 可在该 job 边界内解析；submit/review/publish 在进入 mutation 前按 stored job tenant/project/site 拒绝跨 scope scoped token；稳定 evidence ref extraction 与 record data projection 位于 `internal/domain/evidence`，schema/job/evidence checks、compute record mapping、持久化和 review/publish 状态流仍留在 compute；`review` 只允许 `approved` / `rejected`，`publish` 只允许发布已 approved 的 explanation。该流程不生成解释内容、不执行生产审批、不发布生产指令。
