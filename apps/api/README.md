...
18. `POST /api/v1/simulation-inputs` 可登记 schema-valid `simulation_input.v1`，并按 `simulation_input_id` + payload hash 做幂等；`GET /api/v1/simulation-inputs/{simulation_input_id}` 返回登记记录和原始 payload，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。稳定 simulation input record data projection 位于 `internal/domain/simulation`；schema validation、compute DTO 映射、store persistence、idempotency 和 HTTP mapping 仍留在 compute。
