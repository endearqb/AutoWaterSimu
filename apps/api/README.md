...
| `internal/domain/` | Go API 领域 package，目前包含 agent constraint application plan helper、artifacts retention policy/action planner、evidence input/ref/risk parsing/stored summary risk projection/result explanation refs/readiness policy、jobs status/worker result completion/claim invariants、models compute_result model_run extraction、model_run parsing/check、benchmark_run parsing、benchmark workflow gates、benchmark case readiness、parameter-set status rules、promotion gate 与 production governance gate policy、simulation execution/profile/process-graph helpers 和 workers domain package |
...
Phase 4A 仍以 `internal/compute` 作为主要 compute compatibility wiring package，但已开始 domain package movement：constraint application advisory plan 组装位于 `internal/domain/agent`；artifact retention policy 解析、候选判断和 sweep action planning 位于 `internal/domain/artifacts`；evidence input/ref/risk parsing、stored result summary risk projection、result explanation evidence ref extraction 与 production-readiness policy evaluation 位于 `internal/domain/evidence`；job status 常量、终态/worker-result 状态不变量、worker result completion status/error 提取规则、worker claim capability/contract-version 匹配规则位于 `internal/domain/jobs`；`compute_result.v1.runtime_audit.model_runs` 提取预检、`model_run.v1` 原始 identity/evidence refs/warnings/parameter_hash 解析、model run identity/hash 比对、`benchmark_run.v1` evidence refs 解析、benchmark case scheduling / benchmark run admission gate、单个 benchmark case promotion readiness、default parameter set status 常量/迁移不变量、promotion gate 判定与 model_run production governance gate 判定位于 `internal/domain/models`；simulation job type execution profile / worker capability 映射与 material-balance ProcessGraph-to-SimulationInput projection 位于 `internal/domain/simulation`；worker register / claim / heartbeat 领域服务位于 `internal/domain/workers`，由 `internal/compute` 的 adapter 连接现有 `WorkerStore` 和 job state 投影。platform package movement 也已开始：static bearer token Authenticator 位于 `internal/platform/auth`，runtime `Config` 位于 `internal/platform/config`，contract schema validator 与 base document validation response 位于 `internal/platform/contracts`，平台 HTTP helper 位于 `internal/platform/httpx`，metrics snapshot / read-only collector / Prometheus renderer 位于 `internal/platform/metrics`。`internal/domain/*` 与 `internal/platform/*` packages 不应反向依赖 compute package。当前 metadata `Store` 已先由 12 个领域子接口组合，job create/list/cancel/complete/fail/timeout 已委派到 `JobLifecycleService`；artifact upload / listing / metadata lookup / download / retention / archive 逻辑已委派到 `ArtifactLifecycleService`；worker register / claim / heartbeat 已委派到 `domain/workers.WorkerLifecycleService`；simulation input / process graph 注册与 `simulation_request.input_ref` 解析已委派到只依赖 simulation input、process graph、model run replay 读取接口的 `SimulationInputService`；draft confirmation / promotion 已委派到 `DraftWorkflowService`；result explanation submit/review/publish 已委派到 `ResultExplanationService`；model catalog / benchmark run / model run governance 查询与 benchmark case queueing 已委派到 `ModelGovernanceService`；result read / evidence package / production readiness / evidence-ref resolution 已委派到 `EvidenceGovernanceService`；metrics snapshot collection 已委派到 platform `MetricsService`，具体 Memory/Postgres count 查询仍由 compute store 实现。主 `Service` 主要保留 package-level wiring 与兼容委派。使用 `scripts/audit-compute-api-boundary.ps1` 验证 Store/domain 子接口、内部 service constructor 边界、package boundary、调用来源与实现覆盖。
...
11. `POST /api/v1/contracts/validate` 只校验已有 `contracts/` schema（当前包括 `simulation_request.v1`、`agent_scenario_draft.v1`、`constraint_draft.v1`、`draft_confirmation.v1`、`result_explanation.v1`、`model_catalog.v1` 等）；未知 future schema 必须返回 invalid，不得伪造支持。`POST /api/v1/contracts/confirm-draft` 会校验确认记录和嵌入草案，并持久化确认审计记录；它仍不创建 job、不执行审批或生产动作。`GET /api/v1/contracts/confirmations/{confirmation_id}` 用于只读审计查询。`GET /api/v1/contracts/confirmations/{confirmation_id}/constraint-application-plan` 只对 approved `constraint_draft.v1` 生效，返回 advisory plan，且必须保持 `would_create_job=false` / `would_modify_target=false`；稳定 plan shape 与 warnings 位于 `internal/domain/agent`。`POST /api/v1/contracts/confirmations/{confirmation_id}/promote-simulation-check` 只对 approved `agent_scenario_draft.v1` 生效，且只在嵌入 `proposed_request` 本身是完整 `simulation_request.v1` 时创建 simulation check job。
