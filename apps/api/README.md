...
13. `POST /api/v1/model-catalog/{model_key}/versions/{model_version}/benchmark-runs` 可记录 schema-valid `benchmark_run.v1`，要求 benchmark case 已存在且为 `validated`、parameter set 匹配当前 default parameter set、model_run 已存在且 parameter hash 匹配，并要求 evidence refs 可在该 job 内解析。该记录是执行历史，不创建或执行 benchmark job、不改变参数集状态、不代表生产审批完成。`GET` 列表支持按 `job_id`、`benchmark_case_id` 与 `parameter_set_id` 过滤，scoped token 必须提供已授权 `job_id`，promotion plan 使用同一历史记录面。
