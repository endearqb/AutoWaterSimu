...
17. `POST /api/v1/process-graphs` 可登记 schema-valid `process_graph.v1`，并按 `(process_graph_id, version)` + payload hash 做幂等；首次登记会写 compact `process_graph.registered` audit event，重复同 hash 幂等请求不重复写 audit；`GET /api/v1/process-graphs/{process_graph_id}?version=1` 返回登记记录和原始 payload，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。省略 `version` 时默认读取 version 1。稳定 process graph record data projection 位于 `internal/domain/simulation`；schema validation、compute DTO 映射、store persistence、idempotency 和 HTTP mapping 仍留在 compute。
18. `POST /api/v1/simulation-inputs` 可登记 schema-valid `simulation_input.v1`，并按 `simulation_input_id` + payload hash 做幂等；首次登记会写 compact `simulation_input.registered` audit event，重复同 hash 幂等请求不重复写 audit；`GET /api/v1/simulation-inputs/{simulation_input_id}` 返回登记记录和原始 payload，并按记录中的 `tenant_id` / `project_id` / `site_id` 元数据执行 scoped-token read-scope 拒绝。稳定 simulation input record data projection 位于 `internal/domain/simulation`；schema validation、compute DTO 映射、store persistence、idempotency 和 HTTP mapping 仍留在 compute。
