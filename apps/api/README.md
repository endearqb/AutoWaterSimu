...
5. P0 auth 使用静态 Bearer token + scope，并支持配置级 `revoked` token 拒绝；不实现完整 RBAC 或动态 token 管理 API。`cmd/compute-api` 在 `APP_ENV=production` 或 `ENVIRONMENT=production` 时必须拒绝空 `COMPUTE_API_TOKENS_JSON` 和默认 dev token（`dev-public-token`、`dev-worker-token`、`dev-admin-token`）。Result explanation review/publish 使用 `explanation:write`，避免复用 job 创建权限表达解释发布。当前仅 job create/queue 与 artifact retention delete/archive events 写入 selected mutation audit envelope；完整 all-mutation audit、OIDC/RBAC 和 tenant/project/site data scope 仍是后续安全工作。
