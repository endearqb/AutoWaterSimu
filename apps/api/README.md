...
| `internal/domain/` | Go API 领域 package，目前包含 artifacts retention policy、evidence input/ref/risk parsing/result explanation refs/readiness policy、jobs status/claim invariants、models model_run parsing/check、benchmark_run parsing、benchmark case readiness、parameter-set status rules 与 promotion gate policy、simulation execution/profile/process-graph helpers 和 workers domain package |
