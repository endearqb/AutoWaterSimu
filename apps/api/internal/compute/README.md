...
- Token-scoped tenant/project read filtering for job list/get and artifact download。
...
| `worker_lifecycle.go` | worker register, claim, and heartbeat flow with narrow worker-store dependency |
...
9. Static token rotation is represented by overlapping token config and setting old token records to `revoked=true`; revoked tokens must fail authentication even if their scopes match. Token config may include `tenant_id` and `project_id`; when present, job list/get and artifact download HTTP reads must be constrained to matching job context. Empty tenant/project remains global for existing dev/admin tokens. `site_id` is accepted in token config for forward compatibility but is not enforced until job/site metadata exists. Explanation review/publish mutations require `explanation:write` in addition to the route-level read boundary.
...
21. Artifact upload, job artifact listing, artifact metadata lookup, download, retention sweep, and archive copy/checksum/delete logic live in `ArtifactLifecycleService`; its constructor should stay limited to `JobStore`, `ArtifactMetadataStore`, `ArchiveMetadataStore`, hot artifact storage, archive storage, the contract validator, and a clock.
22. Simulation input registration, process graph registration, `simulation_request.input_ref` resolution, material-balance ProcessGraph-to-SimulationInput transformation, and model-run replay input lookup live in `SimulationInputService`; its constructor should stay limited to `SimulationInputStore`, `ProcessGraphStore`, `ModelRunStore`, `JobStore`, the contract validator, and a clock.
...
25. Model catalog reads/writes, default parameter set status/promotion planning, benchmark run history, model run lookup, and benchmark case queueing live in `ModelGovernanceService`; its constructor should stay limited to `ModelCatalogStore`, `BenchmarkRunStore`, `ModelRunStore`, the contract validator, a clock, `SimulationInputService.ResolveSimulationInput`, the existing compute-job creation callback, and the existing job-scoped evidence resolver callback.
26. Worker register, claim, and heartbeat live in `WorkerLifecycleService`; its constructor should stay limited to `WorkerStore` and a clock. Worker artifact upload and succeed/fail completion remain separate job/artifact lifecycle paths.
27. Result read, evidence package export, production readiness, and evidence-ref resolution live in `EvidenceGovernanceService`; its constructor should stay limited to `JobStore`, `ModelRunStore`, `ProcessGraphStore`, the contract validator, a clock, model catalog resolver callback, artifact listing callback, and artifact metadata callback. It must remain read-only and must not mutate jobs, model catalog snapshots, artifacts, or approval state.
