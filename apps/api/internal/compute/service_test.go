...
func TestHTTPModelCatalogTenantProjectSiteScope(t *testing.T) {
	...
	if rec.Code != http.StatusForbidden {
		t.Fatalf("scoped promotion plan should be denied until job-scoped evidence filtering exists, got %d %s", rec.Code, rec.Body.String())
	}
}
...
func TestHTTPModelCatalogMutationTenantProjectSiteScope(t *testing.T) {
	...
	if rec.Code != http.StatusForbidden {
		t.Fatalf("scoped promote-approved should be denied until job-scoped evidence filtering exists, got %d %s", rec.Code, rec.Body.String())
	}
}
