...
- 已稳定的 `model_run.v1` identity、evidence refs、warnings、parameter_hash 解析规则，`benchmark_run.v1` evidence refs 解析规则，以及 default parameter set status 常量和迁移不变量。
...
| `models/` | model_run identity/ref/warning helpers, benchmark_run evidence ref helpers, and parameter-set status invariants |
