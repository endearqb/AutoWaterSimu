...
- 已稳定的 `model_run.v1` identity、evidence refs、warnings、parameter_hash 解析与 identity/hash 比对规则，`benchmark_run.v1` evidence refs 解析规则，default parameter set status 常量/迁移不变量，以及 default parameter set promotion gate 判定。
...
| `models/` | model_run identity/ref/warning/check helpers, benchmark_run evidence ref helpers, parameter-set status invariants, and promotion gate policy |
