...
| 文件 | 作用 |
|---|---|
| `jobs.go` | Job status constants, worker claim matching, and invariant helpers |
| `jobs_test.go` | Direct jobs domain tests |

## 3. 维护约定

1. 本 package 不得 import `apps/api/internal/compute`。
2. 只放稳定 job domain 不变量；涉及队列选择、状态写入、存储、audit、artifact、result summary 的逻辑继续由 compute compatibility package 承接，直到对应边界可安全迁移。
