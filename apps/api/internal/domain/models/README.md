...
- model version / benchmark case / benchmark run status 常量、单个 benchmark case promotion evidence/result 判定、default parameter set promotion gate 判定，以及 model_run production governance gate 判定。
...
| 文件 | 作用 |
|---|---|
| `models.go` | built-in model catalog document helper, benchmark case run job document helper, model_run identity/ref/warning helpers, compute result model_run extraction helper, model_run identity check helpers, benchmark_run evidence ref helpers, benchmark workflow gates, benchmark case readiness primitive, parameter-set status invariants, promotion gate policy, and production governance gate |
...
- `BenchmarkCasePromotionBenchmarkRun`
- `BenchmarkCasePromotionEvidenceInput`
- `BenchmarkCasePromotionEvidence`
- `EvaluateBenchmarkCasePromotionEvidence`
