...
| `audit-simulation-core-input-contract.ps1` | simulation_core input contract 只读审计，检查 `simulation_input.v1` 顶层与 node/edge item schema 闭合度、`NodeData` / `EdgeData` runtime `extra` 策略、adapter 对未知字段的实际接受/丢弃/告警行为，并输出 `tmp/architecture-evidence/simulation-core-input-contract.json`；默认只对脚本/探针硬违规失败，`-FailOnOpenGaps` 可把开放缺口升级为失败 |
