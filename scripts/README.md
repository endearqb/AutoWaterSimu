...
| `audit-simulation-core-boundary.ps1` | simulation_core / simulation-worker / legacy backend 边界只读审计，检查 `simulation_core/python` 与 `contracts/python` packaging metadata、worker repo-path fallback、runtime `app.*` 反向 import、backend/core 双实现漂移风险、backend oracle 测试依赖和 core-only import smoke，并输出 `tmp/architecture-evidence/simulation-core-boundary.json`；默认只对硬违规失败，`-FailOnOpenGaps` 可把开放缺口升级为失败 |
