...
| `audit-compute-api-boundary.ps1` | Compute API Store/domain 边界只读审计，解析聚合 Store 嵌入接口、内部 domain/platform service constructor 边界、internal package boundary（含已拆出的 artifacts/jobs/models/workers domain 与 platform helpers）、通过 aggregate/narrow repository 字段发起的 resolved Store method 调用来源，并输出 `tmp/architecture-evidence/compute-api-boundary.json` |
