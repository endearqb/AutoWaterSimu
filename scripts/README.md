...
| `audit-compute-api-boundary.ps1` | Compute API Store/domain 边界只读审计，解析聚合 Store 嵌入接口、内部 domain/platform service constructor 边界、internal package boundary（含已拆出的 agent/artifacts/evidence/jobs/models/simulation/workers domain helpers 与 platform helpers；models 包含 built-in catalog document、benchmark case run job document、model_run / benchmark helpers；simulation 包含 execution profile、simulation check job document 和 process graph helpers）、通过 aggregate/narrow repository 字段发起的 resolved Store method 调用来源，并输出 `tmp/architecture-evidence/compute-api-boundary.json` |
