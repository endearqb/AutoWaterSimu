...
| `audit-compute-api-boundary.ps1` | Compute API Store/domain 边界只读审计，从 `store_interfaces.go` 解析聚合 Store 嵌入接口、从 `memory_*.go` / `postgres*.go` 扫描 store implementation coverage（含 `MutationAuditStore`、workspace store、UDM store）、检查内部 domain/platform service constructor 边界、internal package boundary 与 Go import reverse-dependency boundary（含已拆出的 agent/artifacts/evidence/jobs/models/simulation/workers domain helpers 与 platform helpers；models 包含 built-in catalog document、benchmark case run job document、model_run / benchmark helpers；simulation 包含 execution profile、simulation check job document 和 process graph helpers）、通过 aggregate/narrow repository 字段发起的 resolved Store method 调用来源，并输出 `tmp/architecture-evidence/compute-api-boundary.json` |
| `audit-frontend-standalone-compute-boundary.ps1` | 前端 standalone compute 边界只读审计，检查 `frontend/src`（排除 generated client）不再静态导入或直接调用 legacy FastAPI 计算 SDK 的五类计算服务、不再运行时导入 `websocketService`，并确认 standalone adapter 覆盖五类 `compute_job.v1` job type，输出 `tmp/architecture-evidence/frontend-standalone-compute-boundary.json` |
| `audit-simulation-core-boundary.ps1` | simulation_core / simulation-worker / legacy backend 边界只读审计，检查 `simulation_core/python` 与 `contracts/python` packaging metadata、worker runtime 是否不再通过 repo-path fallback 修改 `sys.path`、runtime `app.*` 反向 import、backend material_balance exceptions 是否为 dependency-backed core re-export、backend material_balance calculator 是否为 dependency-backed core re-export 且有 identity test、backend calculator result model contract 是否由 simulation_core calculator/result 满足且 ASM/UDM services 不再注解 legacy result model、backend material_balance utils helper 是否为 dependency-backed core re-export、backend material_balance ASM/UDM runtime helper import paths 是否为 dependency-backed core re-export、`app.material_balance.models` legacy import path 是否 re-export simulation_core runtime models 且 `app.material_balance` 包根不再导出 runtime models、legacy service calculate entrypoints 是否在调用 calculator 前将 `app.models.MaterialBalanceInput` 重新校验为 simulation_core runtime input、legacy direct route schema 是否暴露 `customParameters` / `component_schema` metadata 且 service 入核 helper 会桥接到 core `original_flowchart_data`、backend material_balance calculator 生产入口与 input/adapter boundary 是否显式、backend calculator delegation preflight 是否覆盖 material_balance/ASM1Slim/ASM1/ASM3/UDM fixtures 且无未声明 migration differences、backend/core 双实现漂移风险是否由 calculator thin-shell 或 backend-side delegation preflight + core-only f64 golden guard 控制、backend oracle 测试是否隔离到 backend-dependent lane，以及 core-only import smoke，并输出 `tmp/architecture-evidence/simulation-core-boundary.json`；默认只对硬违规失败，`-FailOnOpenGaps` 可把开放缺口升级为失败 |
