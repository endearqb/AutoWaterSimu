...
| `audit-frontend-standalone-compute-boundary.ps1` | 前端 standalone compute/runtime 边界只读审计，检查 `frontend/src`（排除 generated client）不再静态导入或直接调用 legacy FastAPI 计算 SDK 的五类计算服务、不再运行时导入 `websocketService`，确认 standalone adapter 覆盖五类 `compute_job.v1` job type，并从 `main.tsx` / `standaloneRouteTree.tsx` 静态 import 图检查 standalone runtime 不可达 legacy login/signup/reset/users/items/admin/settings route、`useAuth` 或 legacy FastAPI client，输出 `tmp/architecture-evidence/frontend-standalone-compute-boundary.json` |
