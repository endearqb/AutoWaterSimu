...
$report = [ordered]@{
    ...
    notes = @(
        ...
        "The package movement guardrail expects platform helpers under apps/api/internal/platform, Agent draft proposed request and constraint draft advisory helpers under apps/api/internal/domain/agent, artifact retention policy helpers under apps/api/internal/domain/artifacts, evidence input/ref/risk parsing, result explanation ref extraction, and readiness policy helpers under apps/api/internal/domain/evidence, jobs status, failed-worker fallback result, and claim matching invariants under apps/api/internal/domain/jobs, model_run parsing, model_run identity/hash checks, benchmark_run evidence-ref parsing, benchmark case readiness, parameter-set status invariants, and promotion gate policy under apps/api/internal/domain/models, simulation execution profile and process graph projection helpers under apps/api/internal/domain/simulation, and worker lifecycle under apps/api/internal/domain/workers.",
