...
$report = [ordered]@{
    ...
    notes = @(
        ...
        "The package movement guardrail expects platform helpers under apps/api/internal/platform, artifact retention policy helpers under apps/api/internal/domain/artifacts, evidence input/ref/risk parsing, result explanation ref extraction, and readiness policy helpers under apps/api/internal/domain/evidence, jobs status and claim matching invariants under apps/api/internal/domain/jobs, model_run parsing and parameter-set status invariants under apps/api/internal/domain/models, simulation execution profile and process graph projection helpers under apps/api/internal/domain/simulation, and worker lifecycle under apps/api/internal/domain/workers.",
