...
$scenarios = @(
    ...
    (New-Scenario -LaneMap $laneMap -Id "postgresql_migration" -Title "PostgreSQL migration" -EvidenceSources @("integration") -CoveredBy @("Integration smoke starts an isolated PostgreSQL-backed API stack when available.", "Standalone release gate separately verifies PostgreSQL migration rollback when full RC flags are used.") -RemainingGaps @("No hosted migration green run recorded.")),
    (New-Scenario -LaneMap $laneMap -Id "security_permissions" -Title "Security permissions" -EvidenceSources @("security", "pr_fast") -CoveredBy @("Security smoke covers production token guard, token revocation, scope denial, selected audit, and job/registry/model/benchmark/artifact read-scope checks when available.") -RemainingGaps @("Full OIDC/JWKS, RBAC/ABAC, all-object data scope, ontology-backed policy enforcement, and all-mutation audit remain incomplete.", "No hosted security green run recorded.")),
