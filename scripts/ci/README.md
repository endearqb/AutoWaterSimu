...
| `security-smoke.ps1` | 聚合 production token guard、file-mounted token config source、static token revocation、scope denial、artifact admin scope、tenant/project/site read-scope（job、process_graph、simulation_input、draft_confirmation、model_catalog、model_run、benchmark_run、artifact）、model catalog registration/status、benchmark_run registration 与显式 process_graph/simulation_input registry POST mutation data-scope，以及 selected mutation audit envelope（job、artifact retention、result explanation、draft_confirmation、draft promotion、model governance、simulation registry）的 Go checks，并写出 `tmp/ci-evidence/security-smoke.json` |
