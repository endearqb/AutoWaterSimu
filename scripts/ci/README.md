...
| `security-smoke.ps1` | 聚合 production token guard、file-mounted token config source、static token revocation、scope denial、artifact admin scope、tenant/project/site read-scope（job、process_graph、simulation_input、model_run、benchmark_run、artifact）和 selected mutation audit envelope（job、artifact retention、result explanation、draft promotion、model governance、simulation registry）的 Go checks，并写出 `tmp/ci-evidence/security-smoke.json` |
