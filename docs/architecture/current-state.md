...
- `apps/api/internal/compute` is still a large compatibility wiring package. The aggregate `Store` resolves to 41 methods across 12 embedded domain metadata interfaces. Artifact lifecycle including upload/listing, simulation input/process graph, draft workflow, result explanation, model governance, evidence governance, job lifecycle, and metrics paths now have narrowed constructors. Artifact retention policy helpers have moved to `apps/api/internal/domain/artifacts`, job status and worker claim matching invariants have moved to `apps/api/internal/domain/jobs`, stable `model_run.v1` raw parsing has moved to `apps/api/internal/domain/models`, and worker register/claim/heartbeat has moved to `apps/api/internal/domain/workers` behind a compute adapter. The architecture audit fails if internal domain/platform service constructors accept aggregate `Store` or expose more than three store-like parameters. Platform auth/config/contracts/HTTP/metrics helpers, the base contract document validation response, and the read-only metrics collector moved to `apps/api/internal/platform/auth`, `apps/api/internal/platform/config`, `apps/api/internal/platform/contracts`, `apps/api/internal/platform/httpx`, and `apps/api/internal/platform/metrics`, but full domain workflows such as full artifact lifecycle, full models governance, evidence, simulation, agent, and full jobs lifecycle have not moved yet. Package-level wiring, public `NewService` / `NewServiceWithArchive` signatures, and some handler-facing behavior still remain in the large package; `service.go`, `postgres.go`, `http.go`, and `store.go` remain large-file split targets.
