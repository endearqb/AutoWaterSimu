...
- Full production OIDC/RBAC/data-scope policy is not implemented. P0 static tokens remain the current auth model, but production startup now blocks empty token config and default development token values, and security smoke covers token revocation, scope denial, admin-only artifact retention, and selected mutation audit envelopes for job create plus artifact retention delete/archive events. A hosted/manual security workflow exists, but a real GitHub Actions green run is still pending.
