...
- `apps/api/internal/platform/auth` owns static bearer token config/principal parsing, `apps/api/internal/platform/config` owns the command/runtime `Config` shape, `apps/api/internal/platform/httpx` owns JSON response writing and loopback-only local CORS, and `apps/api/internal/platform/metrics` owns metrics snapshot shape plus Prometheus rendering without importing compute domain types.
...
- Root `Justfile`, `scripts/doctor.ps1`, and `scripts/check-deps.ps1` now provide the first Phase 0 task graph and dependency check entry. `check-deps` also guards the new API platform boundary by preventing `apps/api/internal/platform` from importing the compute domain package.
...
- `apps/api/internal/compute` is still a large package. The aggregate `Store` resolves to 41 methods across 12 embedded domain metadata interfaces. Artifact lifecycle including upload/listing, simulation input/process graph, draft workflow, result explanation, model governance, worker register/claim/heartbeat, evidence governance, job lifecycle, and metrics paths now have narrowed constructors, and the architecture audit fails if internal domain service constructors accept aggregate `Store` or expose more than three store-like parameters. Platform auth/config/HTTP/metrics helpers moved to `apps/api/internal/platform/auth`, `apps/api/internal/platform/config`, `apps/api/internal/platform/httpx`, and `apps/api/internal/platform/metrics`, but domain packages such as jobs/artifacts/models/evidence/simulation have not moved yet. Package-level wiring, public `NewService` / `NewServiceWithArchive` signatures, and some handler-facing behavior still remain in the large package; `service.go`, `postgres.go`, `http.go`, and `store.go` remain large-file split targets.
...
3. Continue Go package movement from the first platform auth/config/HTTP/metrics helper packages toward domain packages, handler/package surface reduction, and later public constructor signature narrowing while preserving MemoryStore/PostgresStore coverage.
