...
- `apps/api/internal/domain/workers` is the first true Go API domain package; it owns worker register / claim / heartbeat domain behavior while `compute` keeps only the adapter to existing stores and job state projections.
...
- `scripts/audit-compute-api-boundary.ps1` verifies the current Compute API Store/domain interface shape, internal service constructor boundaries, internal package boundary, resolved Store-call source files, and writes evidence under `tmp/architecture-evidence/compute-api-boundary.json`.
...
- `apps/api/internal/compute` is still a large compatibility wiring package. The aggregate `Store` resolves to 41 methods across 12 embedded domain metadata interfaces. Artifact lifecycle including upload/listing, simulation input/process graph, draft workflow, result explanation, model governance, evidence governance, job lifecycle, and metrics paths now have narrowed constructors, and worker register/claim/heartbeat has moved to `apps/api/internal/domain/workers` behind a compute adapter. The architecture audit fails if internal domain service constructors accept aggregate `Store` or expose more than three store-like parameters. Platform auth/config/contracts/HTTP/metrics helpers moved to `apps/api/internal/platform/auth`, `apps/api/internal/platform/config`, `apps/api/internal/platform/contracts`, `apps/api/internal/platform/httpx`, and `apps/api/internal/platform/metrics`, but domain packages such as jobs/artifacts/models/evidence/simulation have not moved yet. Package-level wiring, public `NewService` / `NewServiceWithArchive` signatures, and some handler-facing behavior still remain in the large package; `service.go`, `postgres.go`, `http.go`, and `store.go` remain large-file split targets.
...
3. Continue Go package movement from `domain/workers` and the first platform auth/config/contracts/HTTP/metrics helper packages toward additional domain packages, handler/package surface reduction, and later public constructor signature narrowing while preserving MemoryStore/PostgresStore coverage.
