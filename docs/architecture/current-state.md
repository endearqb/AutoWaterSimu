...
- Static token config can carry tenant/project read scope; HTTP job list/get and artifact download deny cross-scope reads for scoped tokens.
...
- `scripts/ci/security-smoke.ps1` defines an opt-in security smoke for production token guard, static token revocation, scope-denial, artifact admin-scope checks, selected mutation audit envelopes, and tenant/project read-scope checks for job list/get plus artifact download, writing `tmp/ci-evidence/security-smoke.json`.
...
- Full production OIDC/RBAC/data-scope policy is not implemented. P0 static tokens remain the current auth model, but production startup now blocks empty token config and default development token values, and security smoke covers token revocation, scope denial, admin-only artifact retention, selected mutation audit envelopes for job create plus artifact retention delete/archive events, and tenant/project read-scope on job list/get plus artifact download. Site scope, all-object data scope, and all-mutation audit remain incomplete. A hosted/manual security workflow exists, but a real GitHub Actions green run is still pending.
