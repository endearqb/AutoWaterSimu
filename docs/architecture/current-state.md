...
- `scripts/audit-compute-api-boundary.ps1` verifies the current Compute API Store/domain interface shape, internal service constructor boundaries, resolved Store-call source files, and writes evidence under `tmp/architecture-evidence/compute-api-boundary.json`.
