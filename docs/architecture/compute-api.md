...
- This split keeps endpoint paths, OpenAPI, auth scopes, contracts, database schema, PostgreSQL migrations, model/evidence governance public service methods, and public `Service` method signatures stable; evidence governance model catalog resolution is now job-scoped for scoped jobs instead of always using the latest unscoped/global catalog.
