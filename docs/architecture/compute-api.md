...
`apps/api/internal/domain/models` owns stable `model_run.v1` raw identity extraction, evidence ref extraction, warning extraction, parameter hash extraction, `benchmark_run.v1` evidence ref extraction, and default parameter set status transition invariants. `apps/api/internal/compute` uses it from MemoryStore/PostgresStore persistence, evidence/simulation read paths, model catalog status workflow validation, benchmark run validation, and promotion planning while catalog snapshot mutation, benchmark workflow DTOs, and HTTP response types remain in the compatibility package.
...
| `domain/models` | `model_run.v1` identity/ref/warning parsing helpers, `benchmark_run.v1` evidence ref helpers, and parameter-set status invariants |
...
`apps/api/internal/domain/models` owns stable `model_run.v1` / `benchmark_run.v1` raw parsing and parameter-set status invariants:
...
Compute uses this package through MemoryStore/PostgresStore model-run persistence, evidence/simulation read paths, default-parameter-set status transition validation, benchmark run evidence-ref validation, and promotion planning parameter-hash checks; full model catalog governance, catalog snapshot mutation, benchmark run persistence workflow, promotion workflow orchestration, HTTP mapping, and DTOs have not moved yet.
