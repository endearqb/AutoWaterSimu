...
| `domain/jobs` | Job status constants, failed-worker fallback result construction, worker result completion extraction, worker claim matching, cancel/timeout state lifecycle service, and invariant helpers |
...
`apps/api/internal/domain/jobs` owns stable job status constants, failed-worker fallback result construction, worker result completion extraction, status invariant helpers, worker claim matching, and cancel/timeout state lifecycle:
...
`job_lifecycle.go` now keeps only the service struct, constructor, store dependencies, validator, clock, artifact-listing callback wiring, and the `domain/jobs.JobStateService` dependency. Same-package workflow methods are grouped into `job_lifecycle_create.go`, `job_lifecycle_read.go`, `job_lifecycle_state.go`, and `job_lifecycle_completion.go`; `job_lifecycle_state.go` is now the compute adapter/snapshot layer for domain jobs state lifecycle. HTTP handlers, OpenAPI, auth scopes, contracts, database schema, generated clients, store interfaces, job idempotency/audit events, worker stale checks, result persistence, model-run persistence, timeout behavior, and snapshot assembly are unchanged.
