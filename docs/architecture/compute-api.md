...
`apps/api/internal/domain/evidence` owns stable evidence package input reference extraction, evidence ref parsing, result explanation evidence ref extraction, embedded simulation input payload lookup, `risk_findings` extraction/summary rules, and production-readiness policy evaluation. `apps/api/internal/compute` uses it from evidence package export, result explanation submit validation, evidence-ref resolution, process graph evidence lookup, and production-readiness response assembly while store-backed evidence governance, response DTOs, HTTP behavior, and approval boundaries remain in the compatibility package.
...
`apps/api/internal/domain/models` owns stable `model_run.v1` raw identity extraction, evidence ref extraction, warning extraction, parameter hash extraction, identity/hash comparison against benchmark expectations, `benchmark_run.v1` evidence ref extraction, single benchmark case promotion readiness, default parameter set status transition invariants, and the pure default-parameter-set promotion gate. `apps/api/internal/compute` uses it from MemoryStore/PostgresStore persistence, evidence/simulation read paths, model catalog status workflow validation, benchmark run validation, and promotion planning while catalog snapshot mutation, benchmark query orchestration, workflow DTOs, and HTTP response types remain in the compatibility package.
...
| `domain/evidence` | Evidence input/ref/risk parsing, result explanation ref extraction, and readiness policy helpers |
| `domain/jobs` | Job status constants, worker claim matching, and invariant helpers |
| `domain/models` | `model_run.v1` identity/ref/warning parsing and identity-check helpers, `benchmark_run.v1` evidence ref helpers, benchmark case readiness helper, parameter-set status invariants, and promotion gate policy |
...
| `evidence_governance.go` | 470 | result read, evidence package export, production readiness, and evidence-ref resolution |
...
`apps/api/internal/domain/evidence` owns stable evidence input/ref/risk parsing, result explanation evidence ref extraction, and readiness policy evaluation:
...
Compute uses this package through evidence package export, result explanation submit validation, evidence-ref resolution, process graph evidence lookup, and production-readiness policy evaluation; store-backed evidence governance, evidence package response assembly, production-readiness DTO mapping, model catalog callbacks, artifact/model-run/process-graph metadata reads, and HTTP mapping have not moved yet.
...
`apps/api/internal/domain/models` owns stable `model_run.v1` / `benchmark_run.v1` raw parsing, model-run identity checks, single benchmark case readiness, parameter-set status invariants, and default-parameter-set promotion gate policy:
...
Compute uses this package through MemoryStore/PostgresStore model-run persistence, evidence/simulation read paths, default-parameter-set status transition validation, benchmark run evidence-ref validation, benchmark run model-run identity/hash validation, promotion planning per-case readiness checks, and final promotion gate evaluation; full model catalog governance, catalog snapshot mutation, benchmark run persistence workflow, benchmark query orchestration, promotion workflow orchestration, HTTP mapping, and DTOs have not moved yet.
...
- This split has moved platform auth, config, contract validation/document response, HTTP, and metrics helpers/collector into platform packages, artifact retention policy helpers into `domain/artifacts`, evidence input/ref/risk parsing, result explanation evidence ref extraction, and readiness policy evaluation into `domain/evidence`, job status plus claim matching invariants into `domain/jobs`, `model_run.v1` raw parsing, identity/hash checking, `benchmark_run.v1` evidence ref parsing, benchmark case readiness, parameter-set status invariants, and promotion gate policy into `domain/models`, simulation execution profile plus material-balance process graph projection helpers into `domain/simulation`, and worker register/claim/heartbeat into `domain/workers`. Most compute domain files have not moved into domain packages yet.
