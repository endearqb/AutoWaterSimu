...
| `domain/artifacts` | Artifact retention policy parsing, candidate checks, sweep action planning, and archive metadata projection helpers |
...
`apps/api/internal/domain/artifacts` owns stable artifact retention and archive metadata projection helpers:
...
Compute uses this package through upload metadata parsing, MemoryStore retention/metrics candidate checks, retention sweep action selection, and archive metadata record assembly; full artifact lifecycle persistence, archive execution, checksum verification, audit envelopes, object-store abstractions, hot object deletion, download fallback, and HTTP mapping have not moved yet.
