...
- The current security smoke proves production default-token startup guard, file-mounted token config source via `COMPUTE_API_TOKENS_FILE`, static token revocation, scope denial, admin-only artifact retention, and selected mutation audit envelopes for job create plus artifact retention events. It does not yet prove full RBAC, all-object tenant/project/site data scope, all-mutation audit, or real issuer/JWKS integration.
