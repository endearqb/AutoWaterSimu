...
- The current golden scenario summary reads existing lane evidence and reports scenario-level `partial` / `missing` / `blocked` status. It is an evidence map, not a replacement for live scenario execution. PostgreSQL migration coverage is counted only from current integration smoke evidence or a current release gate that actually ran the `postgres migration up/down smoke` step.
