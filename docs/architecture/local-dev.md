...
| `just check-security` | Runs token guard, scope denial, revocation, admin-scope, selected mutation audit, model governance audit, and simulation registry audit checks and writes `tmp/ci-evidence/security-smoke.json` |
...
- The current security smoke proves production default-token startup guard, file-mounted token config source via `COMPUTE_API_TOKENS_FILE`, static token revocation, scope denial, admin-only artifact retention, selected mutation audit envelopes for job create, draft promotion / benchmark schedule-run job creation, artifact retention, result explanation submit/review/publish, compact model governance mutations, and simulation registry registrations, plus tenant/project/site read-scope for job list/get, process_graph get, simulation_input get, model_run get / job-filtered list, benchmark_run get / job-filtered list, and artifact download. It does not yet prove full RBAC, all-object tenant/project/site data scope, all-mutation audit, or real issuer/JWKS integration.
