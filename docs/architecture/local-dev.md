...
| `just pr-fast` | Runs the PR fast lane, including Compute API boundary audit, simulation_core correctness-freeze audit, worker dependency installation audit, and writes `tmp/ci-evidence/pr-fast.json` plus audit evidence under `tmp/ci-evidence/` |
...
| `just audit-worker-dependency-installation` | Runs the source-mode worker dependency installation audit and requires installed helper packages without deprecated repo-path fallback |
...
| `just desktop-release-artifacts-smoke` | Builds real unsigned sidecar/NSIS artifacts, runs packaged worker runtime smoke, release gate `Mode=release`, local artifact verifier, and writes `tmp/ci-evidence/desktop-release-artifacts-smoke.json` |
...
| `just golden-scenarios-refresh-desktop-release` | Runs the real unsigned Desktop release artifact smoke, then writes the 8 golden scenario summary from refreshed release evidence |
...
- The current security smoke proves production default-token startup guard, file-mounted token config source via `COMPUTE_API_TOKENS_FILE`, static token revocation, scope denial, admin-only artifact retention, selected mutation audit envelopes for job create/cancel/timeout, worker registration/claim/heartbeat/artifact upload/result completion, draft confirmation records, draft promotion / benchmark schedule-run job creation, artifact retention, result explanation submit/review/publish, compact model governance mutations, and simulation registry registrations, plus tenant/project/site read-scope for job list/get, process_graph get, simulation_input get, draft confirmation get / constraint plan / promotion, persisted model catalog root/model/snapshot reads, model_run get / job-filtered list, benchmark_run get / job-filtered list, artifact download, and mutation data-scope for direct job create/cancel, artifact retention sweep candidate filtering, worker claim/heartbeat/artifact/completion, confirm-draft record persistence, result explanation submit/review/publish, direct simulation-check create, draft promotion job create, benchmark schedule-run job create, model catalog registration/status/promote, benchmark_run registration, and explicit process_graph/simulation_input registry POST. It does not yet prove full RBAC, all-object tenant/project/site data scope, remaining mutation data-scope, all-mutation audit, or real issuer/JWKS integration.
...
- The current Desktop package smoke proves schema fixtures, source-mode runtime clean export/import, checksum-verified restore, metadata restoration, artifact/model_run refs, support bundle redaction, and Desktop React wrapper typechecking. `desktop-release-artifacts-smoke.ps1` is the heavier local unsigned release artifact lane: it builds the PyInstaller sidecar and NSIS installer, runs packaged worker CLI/runtime/installed-sidecar smoke, verifies a local unsigned artifact bundle, and runs release gate `Mode=release` with real artifact paths. It does not prove GitHub hosted artifact upload/download round trip, signing, auto update, or publication.
