...
本轮已完成 backend thin-shell 的两个低风险 leaf：`backend/app/material_balance/exceptions.py` 现在是 `autowatersimu_simulation_core.material_balance.exceptions` 的 compatibility re-export，backend `pyproject.toml` / `uv.lock` 声明本地 `autowatersimu-simulation-core` 依赖，`backend/app/material_balance/core.py` 现在构造 `autowatersimu_simulation_core.material_balance.models.MaterialBalanceResult`，ASM/UDM services 的返回类型注解也改向该 core result model，focused tests 证明 backend/core exception class identity 与 calculator result class identity，`scripts/audit-simulation-core-boundary.ps1` 也会检查这两个 leaf。legacy `core.py` 仍保留 calculator implementation，`models.py` 的旧输入模型、`utils.py`、ASM/UDM helper 也仍未薄壳化，不应把这些 leaf 误读为 PR-31 主迁移完成。

下一次实际执行优先级：

1. 以 P3 green audits、backend/core parity drift guard、input-contract passed evidence、correctness-freeze evidence、exceptions thin-shell leaf 和 result model thin-shell leaf 为基线，下一步继续做 backend thin-shell planning；优先评估 dead `utils.py` / old input model、显式 adapter 校验或 calculator delegation 这些剩余切片，暂不改变 worker 默认 validation mode，也不做 UDM RHS 去循环、dense/sparse 重写、keyset cursor 或 claim LIMIT 热路径优化。
