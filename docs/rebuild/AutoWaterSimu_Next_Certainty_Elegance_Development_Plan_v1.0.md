...
1. 以 P3 green audits、backend/core parity drift guard、input-contract passed evidence、correctness-freeze evidence、calculator/exception/result/utils thin-shell、backend input/adapter boundary、backend calculator delegation preflight 和 worker dependency source-mode gate 为基线，下一步优先做 old compatibility model/helper cleanup 或 benchmark/timings Phase 0 baseline；暂不整文件 re-export `models.py`，不删除 worker deprecated fallback，不改变 worker 默认 validation mode，也不做 UDM RHS 去循环、dense/sparse 重写、keyset cursor 或 claim LIMIT 热路径优化。
