...
本轮已完成 backend thin-shell 的三个低风险 leaf、input/adapter boundary 前置与 calculator delegation preflight：`backend/app/material_balance/exceptions.py` 现在是 `autowatersimu_simulation_core.material_balance.exceptions` 的 compatibility re-export，backend `pyproject.toml` / `uv.lock` 声明本地 `autowatersimu-simulation-core` 依赖，`backend/app/material_balance/core.py` 现在以 `autowatersimu_simulation_core.material_balance.models.MaterialBalanceInput` 作为显式 runtime input annotation 并构造 `MaterialBalanceResult`，ASM/UDM services 的返回类型注解也改向该 core result model，`backend/app/material_balance/utils.py` 现在 re-export `autowatersimu_simulation_core.material_balance.utils`，`backend/app/services/simulation_input_adapter.py` 新增 `simulation_input_to_core_material_balance_input()` 作为显式进入 simulation_core runtime model 的 adapter，并把旧 `simulation_input_to_material_balance_input()` 保留为 legacy `app.models.MaterialBalanceInput` compatibility-only path。focused tests 证明 backend/core exception class identity、calculator result class identity、utility helper object identity、backend-to-core adapter construction，以及 material_balance、ASM1Slim、ASM1、ASM3、UDM fixtures 的 backend calculator delegation preflight；`scripts/audit-simulation-core-boundary.ps1` 也会检查这些 leaf、生产 calculate entrypoints、input/adapter boundary 和 delegation preflight。legacy `core.py` 仍保留 calculator implementation，`models.py` 的旧本地输入模型、ASM/UDM helper 也仍未薄壳化，不应把这些 leaf、input annotation 或 preflight 误读为 PR-31 主迁移完成。

下一次实际执行优先级：

1. 以 P3 green audits、backend/core parity drift guard、input-contract passed evidence、correctness-freeze evidence、exceptions thin-shell leaf、result model thin-shell leaf、utils thin-shell leaf、backend input/adapter boundary 和 backend calculator delegation preflight 为基线，下一步做小切片 `backend-calculator-thin-shell-delegation`：让 backend calculator 委托 simulation_core calculator，同时保留 preflight/parity 守卫；暂不删除 legacy `core.py` 文件、不整文件 re-export `models.py`，不改变 worker 默认 validation mode，也不做 UDM RHS 去循环、dense/sparse 重写、keyset cursor 或 claim LIMIT 热路径优化。
