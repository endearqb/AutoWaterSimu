...
- [ ] Security scope/audit scenario（token guard / file-mounted token config source / scope denial / revocation / selected mutation audit smoke（job、draft promotion、benchmark schedule-run、artifact retention、result explanation、model governance）/ job-model_run-artifact tenant/project/site read-scope 与 hosted/manual workflow 已落地；全对象 data scope 与 all-mutation audit 尚未完成）
...
4. 继续 production security：`COMPUTE_API_TOKENS_FILE` 已补齐 file-mounted service-token config source，result explanation submit/review/publish、draft promotion / benchmark schedule-run job create、model catalog registration、default parameter set status/promote 与 benchmark_run registration 已纳入 selected mutation audit，model_run get / job-filtered list 已纳入 tenant/project/site read-scope；后续继续补 issuer/JWKS 或更深的 service-token secret manager、全对象 data scope、remaining all-mutation audit，并把 security smoke 扩展到这些场景。
