...
- [x] `pr-fast`
- [ ] `integration`
- [x] `browser`（mock-backed opt-in、live Compute backend read opt-in 与 current-flow live opt-in 已落地；完整 legacy authenticated browser session 与 hosted green evidence 尚未完成）
- [ ] `release-evidence`
- [x] `nightly`（scheduled/manual orchestrator 已接入 pr-fast、integration、browser、current-flow live、security、Desktop package smoke；真实 hosted nightly green 尚未完成）
...
3. 触发 current-flow live hosted/manual workflow evidence，将 live backend browser read lane 也提升到 dedicated hosted/manual workflow，并继续补完整 legacy authenticated backend browser session。
