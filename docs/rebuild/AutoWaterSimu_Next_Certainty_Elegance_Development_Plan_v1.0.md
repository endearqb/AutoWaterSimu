...
本轮已完成 backend thin-shell 的第一个低风险 leaf：`backend/app/material_balance/exceptions.py` 现在是 `autowatersimu_simulation_core.material_balance.exceptions` 的 compatibility re-export，backend `pyproject.toml` / `uv.lock` 声明本地 `autowatersimu-simulation-core` 依赖，`backend/app/tests/material_balance_exceptions_thin_shell_test.py` 证明 backend/core exception class identity，`scripts/audit-simulation-core-boundary.ps1` 也会检查该 leaf。`core.py`、`models.py`、`utils.py`、ASM/UDM helper 仍未薄壳化，不应把本 leaf 误读为 PR-31 主迁移完成。

下一次实际执行优先级：

1. 以 P3 green audits、backend/core parity drift guard、input-contract passed evidence、correctness-freeze evidence 和 exceptions thin-shell leaf 为基线，下一步继续做 backend thin-shell planning；优先评估 `MaterialBalanceResult` import、dead `utils.py` / old input model、显式 adapter 校验这些剩余切片，暂不改变 worker 默认 validation mode，也不做 UDM RHS 去循环、dense/sparse 重写、keyset cursor 或 claim LIMIT 热路径优化。
