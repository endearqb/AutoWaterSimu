...
1. 以 P3 green audits、backend/core parity drift guard、input-contract passed evidence 和 correctness-freeze evidence 为基线，下一步优先做 backend thin-shell planning；暂不改变 worker 默认 validation mode，也不做 UDM RHS 去循环、dense/sparse 重写、keyset cursor 或 claim LIMIT 热路径优化。
