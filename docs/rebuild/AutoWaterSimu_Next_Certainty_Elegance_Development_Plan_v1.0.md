...
- [ ] Security scope/audit scenario（token guard / file-mounted token config source / scope denial / revocation / selected mutation audit smoke / job-artifact tenant/project/site read-scope 与 hosted/manual workflow 已落地；全对象 data scope 与 all-mutation audit 尚未完成）
...
4. 继续 production security：`COMPUTE_API_TOKENS_FILE` 已补齐 file-mounted service-token config source；后续继续补 issuer/JWKS 或更深的 service-token secret manager、全对象 data scope、all-mutation audit，并把 security smoke 扩展到这些场景。
