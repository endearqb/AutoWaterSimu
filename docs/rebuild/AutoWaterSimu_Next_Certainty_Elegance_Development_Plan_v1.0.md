...
10. Production dev token 禁用（`cmd/compute-api` 启动 guard 已落地，`COMPUTE_API_TOKENS_FILE` file-mounted token config source 已落地，`scripts/ci/security-smoke.ps1` 已覆盖 token guard / file source / scope denial / revocation / worker mutation POST-only method guard / job collection 与 job route declared-method guard / API-wide declared-method guard / selected mutation audit（job create/cancel/timeout、worker registration/claim/heartbeat/artifact/succeed completion/fail completion、draft confirmation、draft promotion、direct simulation-check、benchmark schedule-run、artifact retention delete/archive、result explanation、model governance、simulation registry）/ job-process_graph-simulation_input-draft_confirmation-model_catalog-model_run-benchmark_run-artifact tenant/project/site read-scope / evidence package 与 production-readiness job-scoped model catalog selection / process_graph evidence-ref source-job object scope / direct job create、direct job cancel、artifact retention sweep candidate filtering 与 archive cross-scope no-write、worker claim/heartbeat/artifact/succeed/fail completion、confirm-draft record persistence、result explanation submit/review/publish、direct simulation-check create、draft promotion job create、benchmark schedule-run job create、model catalog registration-status-promote、promotion plan/promote-approved job-scoped evidence filtering、benchmark_run registration 与显式 process_graph/simulation_input registry POST mutation data-scope；后续继续做 issuer/JWKS、更深的 service-token secret 管理、全对象 data scope、剩余 mutation data-scope、all-mutation audit）。
...
- [ ] Security scope/audit scenario（token guard / file-mounted token config source / scope denial / revocation / worker mutation POST-only method guard / job collection 与 job route declared-method guard / API-wide declared-method guard / selected mutation audit smoke（job create/cancel/timeout、worker registration/claim/heartbeat/artifact/result completion、draft confirmation、draft promotion、direct simulation-check、benchmark schedule-run、artifact retention delete/archive、result explanation、model governance、simulation registry）/ job-process_graph-simulation_input-draft_confirmation-model_catalog-model_run-benchmark_run-artifact tenant/project/site read-scope / evidence package 与 production-readiness job-scoped model catalog selection / process_graph evidence-ref source-job object scope / direct job create、direct job cancel、artifact retention sweep candidate filtering 与 archive cross-scope no-write、worker claim/heartbeat/artifact/completion、confirm-draft record persistence、result explanation submit/review/publish、direct simulation-check create、draft promotion job create、benchmark schedule-run job create、model catalog registration-status-promote、promotion plan/promote-approved job-scoped evidence filtering、benchmark_run registration 与显式 process_graph/simulation_input registry POST mutation data-scope 与 hosted/manual workflow 已落地；全对象 data scope、剩余 mutation data-scope 与 all-mutation audit 尚未完成）
- [x] P2 mutation/data-scope gap matrix（`docs/architecture/compute-mutation-scope-matrix.md` 已逐 route 记录 method guard、auth scope、tenant/project/site mutation scope、no-write denial、audit envelope、security-smoke selection 与 focused test evidence；red/yellow rows 已闭合：route-specific audit smoke alignment、job collection method/no-write proof、direct simulation-check audit proof、artifact archive cross-scope no-write proof；不继续泛化新增 audit helper）
...
- [x] Artifacts compact audit state projection helper package split (`apps/api/internal/domain/artifacts`)
...
- [x] Evidence result explanation compact audit state projection helper package split (`apps/api/internal/domain/evidence`)
...
- [x] Models benchmark case promotion evidence/result helper package split (`apps/api/internal/domain/models`)
...
- [x] Models model catalog snapshot record data projection helper package split (`apps/api/internal/domain/models`)
- [x] Models benchmark run record data projection helper package split (`apps/api/internal/domain/models`)
- [x] Models default parameter set status transition document mutation helper package split (`apps/api/internal/domain/models`)
- [x] Models compact audit state projection helper package split (`apps/api/internal/domain/models`)
...
近期 `.ai/changes` 已显示 evidence wrapper / hosted workflow entry、同 package 文件拆分和普通 service-test split 的边际收益下降。当前 P0 service-test split 已结构性收口，P1 Compute API boundary audit 已进入默认 `pr-fast` gate，P2 mutation/data-scope gap matrix 已落到 `docs/architecture/compute-mutation-scope-matrix.md`，且 red/yellow rows 已由 focused tests 与 `security-smoke` 选集闭合。P3 `simulation-core-packaging-audit` 已推进到可复跑 green baseline：`scripts/audit-simulation-core-boundary.ps1` 会检查 `simulation_core` / worker / legacy backend packaging 与边界，输出 `tmp/architecture-evidence/simulation-core-boundary.json`，并把 runtime hard violation 与开放缺口分开。当前审计基线为 `passed`，0 hard violations、0 open gaps；`simulation_core/python` 与 `contracts/python` packaging metadata、editable install smoke、worker installed-package import preference、deprecated gated repo-path fallback、core-only import/adapter test split、worker CLI/API bridge tests 与 backend oracle tests 隔离，以及 backend/core 双实现 parity drift guard 均已落地。legacy backend 与 `simulation_core` 仍保留重复 material-balance runtime 文件，但该状态现在由 `simulation_core/tests/test_material_balance_core.py` 的显式 `BACKEND_CORE_DRIFT_GUARD_CASES` 覆盖，直到后续 backend thin-shell 迁移。`simulation-core-input-contract-audit` 也已建立并收口：`scripts/audit-simulation-core-input-contract.ps1` 输出 `tmp/architecture-evidence/simulation-core-input-contract.json`，当前为 `passed`，0 hard violations、0 open gaps；`simulation_input.v1` 已关闭 top-level 和 node/edge item unknown fields，node/edge 合同字段为 explicit canonical snake_case，组件/模型内部 map 保持动态结构；`NodeData` / `EdgeData` runtime models 已使用 `extra=forbid` 拒绝直接构造时的未知字段；adapter 默认 `compat` 仍保留兼容性静默忽略行为，opt-in `warn` 会把未知字段写入 `MaterialBalanceInput.contract_warnings`，opt-in `strict` 会在计算前拒绝未知字段。`simulation-core-correctness-freeze` 也已建立：`scripts/audit-simulation-core-correctness-freeze.ps1` 输出 `tmp/architecture-evidence/simulation-core-correctness-freeze.json`，当前为 `passed`，0 hard violations、0 open gaps，并冻结 `_run_hours` mixed reaction-model combined dispatcher、single-model fallback 顺序、ASM/UDM 分支 clamp 与 default 分支 commented clamp 状态、ASM 氧清零 active compute mask 范围以及 `compute_mask` derivative masking；mixed-model 最终执行语义已由 ADR 0015 选择 supported dispatch。只有能产出真实 hosted green evidence 时才补 hosted workflow 证据。

本轮已完成 backend calculator thin-shell 主切片及其前置：`backend/app/material_balance/core.py` 现在是 `autowatersimu_simulation_core.material_balance.core.MaterialBalanceCalculator` 的 compatibility re-export，`backend/app/material_balance/exceptions.py` 是 `autowatersimu_simulation_core.material_balance.exceptions` 的 compatibility re-export，backend `pyproject.toml` / `uv.lock` 声明本地 `autowatersimu-simulation-core` 依赖，ASM/UDM services 不再注解 legacy result model，`backend/app/material_balance/utils.py` re-export `autowatersimu_simulation_core.material_balance.utils`，`backend/app/services/simulation_input_adapter.py` 提供 `simulation_input_to_core_material_balance_input()` 作为显式进入 simulation_core runtime model 的 adapter，并把旧 `simulation_input_to_material_balance_input()` 保留为 legacy `app.models.MaterialBalanceInput` compatibility-only path。focused tests 证明 backend/core calculator class identity、exception class identity、calculator result class identity、utility helper object identity、backend-to-core adapter construction，以及 material_balance、ASM1Slim、ASM1、ASM3、UDM fixtures 的 backend calculator delegation preflight；`scripts/audit-simulation-core-boundary.ps1` 也会检查这些 thin-shell 状态、生产 calculate entrypoints、input/adapter boundary 和 delegation preflight。legacy `models.py` 的旧本地输入模型、ASM/UDM helper 仍未薄壳化，不应把 calculator thin shell 误读为整文件 `models.py` re-export、legacy route schema 变更、worker strict mode 变更或热路径性能优化完成。

本轮也完成 source-mode `worker-dependency-installation-gate`：backend `pyproject.toml` / `uv.lock` 同时声明 editable `autowatersimu-contracts` 与 `autowatersimu-simulation-core`；worker `self_check()` 报告 required modules、module locations、missing-before/after-fallback 和 `deprecated_repo_path_fallback_used`；`scripts/audit-worker-dependency-installation.ps1` 与默认 `pr-fast` gate 要求 source-mode self-check 不使用 deprecated repo-path fallback，并输出 `tmp/architecture-evidence/worker-dependency-installation.json` / `tmp/ci-evidence/worker-dependency-installation/worker-dependency-installation.json`。deprecated fallback 仍作为兼容路径和 dedicated fallback test 保留；packaged sidecar fallback 删除证据仍属于后续 package/desktop-adjacent movement，不在本轮完成态内。

本轮已启动并落地 benchmark/timings Phase 0 baseline harness：`scripts/ci/performance-baseline-phase0.ps1` 作为 opt-in lane 运行既有 worker compute_job fixtures 的 solver matrix，覆盖 small material balance、medium ASM1、single UDM 与 `scipy_solver` / `rk4` / `adaptive_heun`，并记录 worker wall time、`compute_result.runtime_audit.timings_ms` 分段、artifact size 以及未来 Go claim/list latency 指标设计。worker 成功结果现在暴露 `schema_validate`、`dependency_import`、`adapter_convert`、`compute`、`artifact_serialize`、`result_envelope` 与 `total` timing segments。当前 baseline evidence 在缺少 tracked `mixed_asm_udm` fixture 时为 `partial` open gap，不把 mixed ASM/UDM 维度伪装为完成；本轮没有做 UDM RHS、dense/sparse、solver output grid、keyset cursor 或 claim LIMIT 等热路径优化，也没有改变 worker default strict mode 或删除 deprecated fallback。

下一次实际执行优先级：

1. 以 P3 green audits、backend/core parity drift guard、input-contract passed evidence、correctness-freeze evidence、calculator/exception/result/utils thin-shell、backend input/adapter boundary、backend calculator delegation preflight、worker dependency source-mode gate 和 Phase 0 baseline harness 为基线，下一步优先补 tracked mixed ASM/UDM benchmark fixture / baseline，或继续 old compatibility model/helper cleanup；暂不整文件 re-export `models.py`，不删除 worker deprecated fallback，不改变 worker 默认 validation mode，也不做 UDM RHS 去循环、dense/sparse 重写、keyset cursor 或 claim LIMIT 热路径优化。
2. 继续保持 worker test boundary 规则：worker runtime/CLI/API bridge tests 不得静态加入 legacy `backend/app`；old-vs-worker 数值基线只能放在 dedicated backend-oracle test file，并由 `scripts/audit-simulation-core-boundary.ps1` 审计。
3. 继续保持 worker import path 规则：优先 installed `autowatersimu-simulation-core` / `autowatersimu-contracts`，repo path mutation 只能保留为 deprecated fallback，并由 `scripts/audit-simulation-core-boundary.ps1` 与 `scripts/audit-worker-dependency-installation.ps1` 审计；默认 CI/source-mode gate 不允许 fallback 成为成功路径。
