...
- [x] Go API domain package split（file-level Store/MemoryStore、HTTP route group、PostgreSQL persistence split、public Service delegate split、artifact lifecycle / simulation input-process graph / draft / model governance / evidence governance workflow split complete；后续随 domain workflow 迁出继续瘦身）
