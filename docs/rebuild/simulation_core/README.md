...
3. 新增性能优化计划前，先确认是否已有 baseline、profiling、golden、hot-path prereview、Go API latency smoke、worker strict smoke、packaged no-fallback smoke 和 audit 证据；截至 2026-06-15，P-01 mixed baseline fixture、P-02 profiling artifacts、P-03 CPU/f64 golden generator（含 KPI-017 N=100 expression cache build-time evidence 与 PR-37 core-only committed f64 golden guard）、P-04 backend compatibility models boundary（`app.material_balance.models` re-export 与 package-root model export removal）、backend app.models service-layer runtime revalidation、backend ASM/UDM runtime helper thin-shell、P-05 worker adapter strict opt-in smoke、P-06 Go API latency smoke、P-07 worker packaged no-fallback evidence 与 worker runtime fallback 删除、P-08 hot-path prereview（含 UDM solver bucket breakdown for KPI-001/KPI-003）、绝对阈值 KPI nightly 固定 runner 策略与 baseline 硬件指纹记录、performance flag matrix evidence（默认/目标值、组合矩阵和退场条件）、第一批 `transport-runtime-tensor-precompute-no-semantics`、第二批 `udm-expression-cache-and-device-sync-reduction`、PR-4 UDM index-conflict/write-side guard、PR-36 default solver no-switch decision（ADR 0016）、PR-37 parity→golden 第一阶段改造、PR-38 supported mixed-model dispatch、PR-39 UDM runtime mapping/stoich mismatch guard first step 与 core ASM schema-driven component gather/scatter、PR-11 unified reaction RHS、PR-32 dense/sparse parallel-edge unification、PR-33 dense lazy / `_balance_param` shape guard、segment timestamp CPU construction、`parameter_names` reuse、PR-34 输出网格解耦、PR-35 真实质量守恒指标与 PR-13a 表达式校验器白名单化已完成。
...
- 未验证的外部聊天结论。
- 未跟踪、未复跑的本地性能数字。
- legacy backend oracle 作为 `simulation_core/tests` 或 Phase 0 golden 的正确性来源；backend 对照应留在 backend-dependent lane。
