...
| 项目 | 规划事项 | 状态 |
| --- | --- | --- |
| 1 | Docker Compose 全链路跑通（含 DB）与国际化 | 进行中 |
| 2 | 清理敏感/多余数据表与中间文件 |  |
...
| 7 | 优化模型添加流程 |  |
| 8 | 增加添加自定义模型功能 |  |
| 9 | 文献数据集 Benchmark Case，与文献曲线对标输出（可复现） |  |
| 10 | 多节点张量化表示 + 分组异构模型（每节点可跑不同模型） |  |
| 11 | 参数扫掠/方案对比（“一张图跑所有选项”） |  |
| 12 | 性能基准与可选 GPU 加速落地 |  |
| 13 | 校准/调参工作流（真实项目落地包） |  |
| 14 | “Vibe Modeling”自然语言建模入口（远期愿景） |  |

| Item | Planned Work | Status |
| --- | --- | --- |
| 1 | Full Docker Compose pipeline running end‑to‑end (including DB) and i18n | In progress |
| 2 | Clean up sensitive/redundant tables and intermediate files |  |
| 3 | One‑click simulation for a minimal example case + smoke test CI |  |
| 4 | Continuous influent (time‑series input) |  |
| 5 | Initial refactor of core module (decouple `core.py` into maintainable structure) |  |
| 6 | Add more open‑source models |  |
| 7 | Improve the model addition workflow |  |
| 8 | Support adding custom user‑defined models |  |
| 9 | Benchmark cases based on literature datasets, with reproducible curve‑matching outputs |  |
| 10 | Tensorized multi‑node representation + grouped heterogeneous models (different model per node) |  |
| 11 | Parameter sweep / scenario comparison (“run all options in one plot”) |  |
| 12 | Performance benchmarks and optional GPU acceleration |  |
| 13 | Calibration/tuning workflow for real‑world project deployment |  |
| 14 | “Vibe Modeling” natural‑language modeling interface (long‑term vision) |  |
