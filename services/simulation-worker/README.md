...
|---|---|
| `README.md` | 本目录上下文契约 |
| `Dockerfile` | Standalone compose worker image，build 阶段预装 worker 依赖，运行时不再 pip install |
...
Phase 2B 后 worker 通过 installable `autowatersimu-simulation-core` / `autowatersimu-contracts` Python packages 调用 material balance runtime，不再直接依赖 `backend/app`。`simulation_core/python` 与 `contracts/python` 不再作为 runtime repo-path fallback 注入 `sys.path`；它们只作为 backend editable dependency 的 source roots 与 packaged sidecar build 输入存在。当前默认 source-mode gate 由 `scripts/audit-worker-dependency-installation.ps1` 和 `pr-fast` 执行，要求 helper packages 可导入且 self-check 中兼容字段 `deprecated_repo_path_fallback_used=false`；P-07 packaged gate 由 `scripts/ci/worker-packaged-no-fallback-smoke.ps1` 执行，要求 PyInstaller one-folder sidecar self-check 同样报告 `deprecated_repo_path_fallback_used=false`。当前 worker 接受 `simulation.material_balance.v1`、`simulation.asm1slim.v1`、`simulation.asm1.v1`、`simulation.asm3.v1` 与 `simulation.udm.v1`；ASM1Slim/ASM1/ASM3/UDM 仍通过既有 material balance runtime 的节点模型分支执行。self-check 声明 `material_balance`、`asm1slim`、`asm1`、`asm3`、`udm`、`ode` capabilities，并报告 worker dependency required modules、module locations、missing-before/after-fallback 兼容字段、deprecated fallback 固定未使用状态、torch runtime 线程配置和当前 adapter validation mode。worker 默认 adapter validation mode 仍为 `compat`；`--adapter-validation-mode strict` 或 `AUTOWATERSIMU_WORKER_ADAPTER_VALIDATION_MODE=strict` 仅作为 opt-in rollout/smoke 入口，当前 strict 默认切换前置证据由 `scripts/ci/worker-adapter-strict-smoke.ps1` 维护。成功 `compute_result.v1.runtime_audit.timings_ms` 当前记录 `schema_validate`、`dependency_import`、`adapter_convert`、`compute`、`artifact_serialize`、`result_envelope` 与 `total`，供 `scripts/ci/performance-baseline-phase0.ps1` 建立 baseline；该 baseline 同时记录硬件指纹和“绝对阈值仅 nightly 固定 runner 判定”的策略，这些字段是观测/证据元数据，不是热路径优化。`--run-api-once` 是 Phase 4/5 的本地/CI HTTP worker bridge，用于 register -> claim -> heartbeat -> run -> upload artifact -> succeed/fail 的单次闭环；`--run-api-loop` 在同一 HTTP contract 上复用一次注册并重复 claim，支持 `--max-jobs` / `--max-idle-polls` / `--idle-sleep-seconds` 做 bounded loop。`--api-token` / `SIMULATION_WORKER_API_TOKEN` 为空时不会发送 Authorization header，用于 standalone `COMPUTE_API_AUTH_MODE=disabled`；传入 token 时保留 static-token compose/security smoke 行为。生产部署编排、异步求解中断和复杂 cancel acknowledgement 仍是后续工作。
...
- `simulation_core/` / `contracts/` 作为 editable dependency source roots 和 packaged sidecar build inputs。
...
backend\.venv\Scripts\python services\simulation-worker\simulation_worker\cli.py --run-api-once --api-base-url http://localhost:8088 --api-token "" --artifact-dir tmp\worker-api-artifacts
