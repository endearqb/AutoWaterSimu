...
- Worker dependency installation gate、installed package import preference 与 no-fallback report 测试。
...
4. 默认 source-mode self-check 应证明 `autowatersimu-contracts` 与 `autowatersimu-simulation-core` 来自已安装依赖；worker runtime 不再提供 repo-path fallback，兼容字段 `deprecated_repo_path_fallback_used` 必须保持 `false`。
