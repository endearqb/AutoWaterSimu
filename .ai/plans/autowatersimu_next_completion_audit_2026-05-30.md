...
- `backend\.venv\Scripts\python -m pytest simulation_core\tests -q`: passed, `7 passed`
...
| Phase 2 | Simulation core and worker CLI | Strong partial | Worker self-check, worker tests, minimal run-job, ASM1Slim model-bound run-job, independent `simulation.asm1slim.v1` run-job, simulation core tests, and API-once worker smoke pass | Expand independent job type support beyond ASM1Slim; add long-running worker daemon/heartbeat coverage when that mode exists |
...
| Phase 5 | ProcessGraph integration and model migration | Strong partial | Contract transforms, current-flow job submission, ProcessGraph registry, ProcessGraph-to-SimulationInput API resolution, ASM1Slim model-bound parity, and independent `simulation.asm1slim.v1` worker/core/backend parity exist | Independent ASM1/ASM3/UDM job type contracts/handlers, full old-vs-worker numerical baseline matrix, Petersen tutorial worker baseline, Playwright flow smoke beyond current minimal path |
...
- Continue migrating independent ASM/UDM long-running workloads to the worker path one job type at a time; `simulation.asm1slim.v1` is the first completed executable contract/worker slice.
...
1. Design and migrate independent ASM1, ASM3, and UDM job types into the worker path with old-vs-worker numerical baseline fixtures.
