...
- `asm1slim_minimal.simulation_input.v1.json` and `asm1slim_minimal.compute_job.v1.json` now exist as model-bound material balance fixtures while contracts still allow only `simulation.material_balance.v1`
...
- `backend\.venv\Scripts\python -m pytest simulation_core\tests -q`: passed, `6 passed`
...
| Phase 2 | Simulation core and worker CLI | Strong partial | Worker self-check, worker tests, minimal run-job, ASM1Slim model-bound run-job, simulation core tests, and API-once worker smoke pass | Expand independent job type support beyond material balance; add long-running worker daemon/heartbeat coverage when that mode exists |
...
| Phase 5 | ProcessGraph integration and model migration | Strong partial | Contract transforms, current-flow job submission, ProcessGraph registry, ProcessGraph-to-SimulationInput API resolution, and ASM1Slim model-bound worker/core/backend parity exist under `simulation.material_balance.v1` | Independent ASM/UDM job type contracts/handlers, full old-vs-worker numerical baseline matrix, Petersen tutorial worker baseline, Playwright flow smoke beyond current minimal path |
...
- Keep ASM1Slim/ASM/ASM3/UDM model-bound nodes passing through the current material balance worker path while independent job type semantics are designed.
- Migrate independent ASM/UDM long-running workloads to the worker path one job type at a time.
- Add old-vs-worker numerical baseline matrix for ASM1Slim, ASM1, ASM3, UDM, and Petersen tutorial flows.
...
1. Design and migrate independent ASM/UDM job types into the worker path with old-vs-worker numerical baseline fixtures.
