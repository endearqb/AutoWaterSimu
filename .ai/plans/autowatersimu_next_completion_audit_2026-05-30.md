...
- Signing, auto-update, and GitHub Release publication are now explicitly post-P0 policy-driven work in `.ai/decisions/0011-desktop-release-signing-auto-update-boundary.md`; live GitHub Windows runner release-build timing/download verification is not yet verified.

## Phase Status

| Phase | Requirement Area | Current Status | Evidence | Remaining Work |
|---|---|---|---|---|
| Phase 0 | Legacy baseline stabilization | Strong partial | Targeted legacy backend tests and frontend type/build pass with existing warnings only; exact legacy `print`/schema/client drift audit exists; route no-print guard scans all route modules; material balance route BOM, commented conversion-service debug prints, Pydantic protected namespace warnings, v1 validator warnings, and FastAPI lifespan deprecation warnings were cleaned | Keep legacy tests as migration baseline; remove debug-script/test print noise only when those files become maintained tooling; legacy OpenAPI metadata refresh and third-party `python_multipart` warning remain separate |
...
| Phase 6.4 | Lifecycle and operations | Strong partial | Artifact retention metadata, admin-scoped manual retention sweep API with safe dry-run default, disabled-by-default scheduler, guarded Compute Jobs retention UI, dedicated Compute lifecycle admin page, store-backed `/metrics` gauges, operations runbook, backup/restore/token runbooks, Prometheus alert rule examples, Alertmanager route example, Grafana dashboard example, monitoring deployment runbook, archive backend boundary ADR, opt-in `local_fs_archive` backend with durable metadata/download fallback, internal service-level retention sweep with model_run reference protection, generated client wrapper, and static token revoke exist | Production object-store archive backend and live monitoring deployment/receiver policy |
...
1. Add production object-store archive backend and live monitoring deployment/receiver wiring for artifact/model-run lifecycle management.
