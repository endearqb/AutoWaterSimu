...
| Phase 6.4 | Lifecycle and operations | Strong partial | Artifact retention metadata, admin-scoped manual retention sweep API with safe dry-run default, internal service-level retention sweep with model_run reference protection, generated client wrapper, and static token revoke exist | Scheduler, archive backend, metrics/SLO hardening, operation runbooks, and lifecycle UI |
...
1. Add scheduler, archive backend decision, metrics/SLOs, and operational runbooks for artifact/model-run lifecycle management.
