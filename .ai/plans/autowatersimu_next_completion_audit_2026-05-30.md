...
| Phase 6.4 | Lifecycle and operations | Strong partial | Artifact retention metadata, admin-scoped manual retention sweep API with safe dry-run default, store-backed `/metrics` gauges, internal service-level retention sweep with model_run reference protection, generated client wrapper, and static token revoke exist | Scheduler, archive backend, SLO/alert hardening, operation runbooks, and lifecycle UI |
...
- Extend operational metrics into SLO/alert rules and scheduler/archive metrics after those lifecycle components exist.
...
1. Add scheduler, archive backend decision, SLO/alert rules, and operational runbooks for artifact/model-run lifecycle management.
