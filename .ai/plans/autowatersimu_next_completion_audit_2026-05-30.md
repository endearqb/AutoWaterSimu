...
| Phase 4 | Web Compute API P0A/P0B | Strong partial | Go API tests pass; API/worker smoke passes; PostgreSQL migration up/down smoke passes locally and has a manual opt-in CI job; generated client and Web build pass; static bearer token scope/revocation runbook exists | Deployment-specific secret-manager wiring; broader browser coverage; live CI migration smoke evidence |
