...
| Phase 6.4 | Lifecycle and operations | Strong partial | Artifact retention metadata, admin-scoped manual retention sweep API with safe dry-run default, disabled-by-default scheduler, guarded Compute Jobs retention UI, store-backed `/metrics` gauges, operations runbook, backup/restore runbook, Prometheus alert rule examples, archive backend boundary ADR, internal service-level retention sweep with model_run reference protection, generated client wrapper, and static token revoke exist | Archive backend implementation, Alertmanager/dashboard deployment, and dedicated lifecycle admin UI |
...
1. Add archive backend implementation, Alertmanager/dashboard deployment wiring, and a dedicated lifecycle admin UI for artifact/model-run lifecycle management.
