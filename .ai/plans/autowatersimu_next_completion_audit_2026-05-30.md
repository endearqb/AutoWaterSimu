...
- `backend\.venv\Scripts\python -m pytest simulation_core\tests -q`: passed, `8 passed`
...
- Continue migrating independent ASM/UDM long-running workloads to the worker path one job type at a time; `simulation.asm1slim.v1` and `simulation.asm1.v1` are completed executable contract/worker slices.
...
1. Design and migrate independent ASM3 and UDM job types into the worker path with old-vs-worker numerical baseline fixtures.
