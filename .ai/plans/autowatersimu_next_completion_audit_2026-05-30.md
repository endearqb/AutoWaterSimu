...
| Phase 6.4 | Lifecycle and operations | Strong partial | Artifact retention metadata, admin-scoped manual retention sweep API with safe dry-run default, guarded Compute Jobs retention UI, store-backed `/metrics` gauges, operations runbook, internal service-level retention sweep with model_run reference protection, generated client wrapper, and static token revoke exist | Scheduler, archive backend, alert manager/dashboard hardening, and dedicated lifecycle admin UI |
...
1. Add scheduler, archive backend decision, executable alert rules/dashboard wiring, and a dedicated lifecycle admin UI for artifact/model-run lifecycle management.
