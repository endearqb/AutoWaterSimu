...
| Phase 3 | Desktop MVP | Strong partial | Desktop Rust tests, Desktop typecheck, and Desktop build pass; project registry/project package export-import/project_id/support bundle commands are present; project package exports include project-scoped jobs, CanvasGraphs, artifact refs and support bundle refs with metadata-only import boundaries; PyInstaller one-folder packaged worker sidecar build, explicit Desktop runtime smoke, Tauri resource-bundled NSIS installer build, and installer smoke pass locally | External file dialog allowlist, recent files, file-backed package restore beyond CanvasGraphs, signing/auto-update decisions |
