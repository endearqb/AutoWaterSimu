...
Accepted on 2026-07-23; amended on 2026-08-10 to distinguish business isolation from shared UI/interaction primitives.
...
- V2 may reuse allowlisted stateless presentation/interaction primitives such as `NodePalette`, `GlassNodeContainer`, glass color utilities, and `EdgeQuickToolbar`.
- Shared primitives receive state and actions through props. They must not own a legacy store, v2 store, business enum, connection rule, serializer, or service call.
- V1 and v2 should preserve the same editor interaction language where the product behavior is equivalent: persistent edge values, inline editing, selection feedback, contextual quick actions, and a full inspector for detailed parameters.
- V2 must not reuse `FlowCanvas`, legacy stores, legacy services, or legacy node/edge business behavior.
...
- Business-model duplication remains intentional while v2 stabilizes; generic UI/interaction duplication should be extracted into allowlisted shared primitives when doing so does not widen the data boundary.
- v1 cleanup happens after the v2 feature owns its local edge model.
- Boundary checks must fail if v2 imports legacy Flow/store/service code outside the presentation/interaction allowlist, or if v1 imports the v2 feature.
...
- UDM-v2 reuses only allowlisted stateless UI/interaction primitives; its canvas orchestration, node/edge semantics, handles, store, serializer, connection validation and services remain feature-local.
