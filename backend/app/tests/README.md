...
| `material_balance_compat_models_boundary_test.py` | `app.material_balance.models` core model re-export and production import boundary guard |
...
7. `app.material_balance.models` 只能作为 simulation_core runtime model 的 compatibility re-export 旧导入路径；生产代码不得重新引入 backend-local input model 副本。
