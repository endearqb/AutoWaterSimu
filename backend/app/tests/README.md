...
| `material_balance_calculator_delegation_preflight_test.py` | backend-side shadow/parity guard for future calculator delegation across material_balance, ASM1Slim, ASM1, ASM3, and UDM fixtures |
...
6. backend calculator thin-shell 主迁移前必须保持 delegation preflight 通过；该 preflight 必须覆盖 material_balance minimal、ASM1Slim model-bound、独立 ASM1Slim/ASM1/ASM3/UDM fixtures，并显式记录 allowed migration differences。
