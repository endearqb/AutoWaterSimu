...
# Initialize services
asm1_service = ASM1Service()
data_conversion_service = DataConversionService()


@router.post("/calculate", response_model=ASM1JobPublic)
def create_calculation_job(
    *,
    ...
    background_tasks.add_task(
        asm1_service.run_calculation,
        session,
        job_id,
        calculation_input
    )
    
    return ASM1JobPublic(
        id=job.id,
        job_id=job.job_id,
...
@router.post("/calculate-from-flowchart", response_model=ASM1JobPublic)
def create_calculation_job_from_flowchart(
    *,
    ...
    try:
        ...
        current_time = datetime.now().strftime("%Y%m%d%H%M")
        job_name = f"asm1_{flowchart_name}_{current_time}"
...
def get_calculation_result_summary(
    *,
    ...
    获取ASM1计算结果摘要
    """
    statement = select(ASM1Job).where(
        ASM1Job.job_id == job_id,
        ASM1Job.owner_id == current_user.id
    )
    ...
    if not job:
        raise HTTPException(
            status_code=404,
            detail="ASM1 calculation job not found"
        )
    
    if job.status != MaterialBalanceJobStatus.success:
        raise HTTPException(
            status_code=400,
            detail=f"ASM1 calculation not completed successfully. Status: {job.status}"
        )
    
    if not job.result_data:
        raise HTTPException(
            status_code=404,
            detail="ASM1 calculation result data not found"
        )
...
@router.get("/status/{job_id}", response_model=ASM1JobPublic)
...
@router.get("/jobs", response_model=ASM1JobsPublic)
def get_user_calculation_jobs(
    *,
    ...
    """
    # Get total count
    count_statement = select(ASM1Job).where(
        ASM1Job.owner_id == current_user.id
    )
    ...
    statement = (
        select(ASM1Job)
        .where(ASM1Job.owner_id == current_user.id)
        .order_by(ASM1Job.created_at.desc())
        .offset(skip)
        .limit(limit)
    )
...
def delete_calculation_job(
    *,
    ...
    return Message(message="ASM1 calculation job deleted successfully")


@router.get("/jobs/{job_id}/input-data", response_model=ASM1JobInputDataResponse)
def get_job_input_data(
    *,
    ...
    return ASM1JobInputDataResponse(
        job_id=job.job_id,
