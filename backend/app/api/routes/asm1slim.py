...
from app.services.asm1slim_service import ASM1SlimService
...
# Initialize services
asm1slim_service = ASM1SlimService()
...
def create_calculation_job(
    *,
    ...
    job_name = f"asm1slim_{current_time}"
    ...
    background_tasks.add_task(
        asm1slim_service.run_calculation,
        session,
        job_id,
        calculation_input
    )
    
    return ASM1SlimJobPublic(
        id=job.id,
        job_id=job.job_id,
...
def create_calculation_job_from_flowchart(
    *,
    ...
    try:
        ...
        current_time = datetime.now().strftime("%Y%m%d%H%M")
        job_name = f"asm1slim_{flowchart_name}_{current_time}"
...
def get_calculation_result_summary(
    *,
    ...
    statement = select(ASM1SlimJob).where(
        ...
        ASM1SlimJob.owner_id == current_user.id
    )
    ...
    if not job:
        raise HTTPException(
            status_code=404,
            detail="ASM1slim calculation job not found"
        )
    
    if job.status != MaterialBalanceJobStatus.success:
        raise HTTPException(
            status_code=400,
            detail=f"ASM1slim calculation not completed successfully. Status: {job.status}"
        )
    
    if not job.result_data:
        raise HTTPException(
            status_code=404,
            detail="ASM1slim calculation result data not found"
        )
...
@router.get("/status/{job_id}", response_model=ASM1SlimJobPublic)
...
def validate_calculation_input(
    *,
    ...
    try:
        ...
        # since ASM1slim uses the same input structure
...
@router.get("/jobs", response_model=ASM1SlimJobsPublic)
def get_user_calculation_jobs(
    *,
    ...
    """
    # Get total count
    count_statement = select(ASM1SlimJob).where(
        ASM1SlimJob.owner_id == current_user.id
    )
    ...
    statement = (
        select(ASM1SlimJob)
        .where(ASM1SlimJob.owner_id == current_user.id)
        .order_by(ASM1SlimJob.created_at.desc())
        .offset(skip)
        .limit(limit)
    )
...
def delete_calculation_job(
    *,
    ...
    return Message(message="ASM1slim calculation job deleted successfully")


@router.get("/jobs/{job_id}/input-data", response_model=ASM1SlimJobInputDataResponse)
def get_job_input_data(
    *,
    ...
    return ASM1SlimJobInputDataResponse(
        job_id=job.job_id,
