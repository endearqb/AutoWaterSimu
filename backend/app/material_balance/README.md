...
6. `models.py` 只保留 compatibility re-export；包根 `app.material_balance` 不再 re-export runtime input/result models。legacy FastAPI route schema 仍来自 `app.models.MaterialBalanceInput`，可携带 `customParameters` / `component_schema` metadata；service 入口必须在调用 calculator 前把它折叠进 `original_flowchart_data` 并重新校验为 `autowatersimu_simulation_core.material_balance.models.MaterialBalanceInput`。
7. `simple_test.py` 与 `test_module.py` 只是旧本地模型的手工兼容脚本，不作为生产入口、pytest baseline 或性能前置证据；新增代码不得依赖它们证明 runtime 合同。
8. `models.py` 兼容路径已 thin-shell 化且是唯一 legacy material_balance runtime model 兼容路径；不得把它误读为 `app.models` 删除、worker strict mode 变更或热路径性能优化完成。
