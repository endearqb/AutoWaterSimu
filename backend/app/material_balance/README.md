...
| 文件/子目录 | 作用 |
|---|---|
| `core.py` | legacy material balance calculator implementation；input annotations and returned result model point to `autowatersimu_simulation_core.material_balance.models` |
...
7. legacy calculator implementation、ASM/UDM helpers 仍未迁移，不得把这些 leaf 或 input annotation change 误读为整文件 re-export 或 PR-31 backend thin-shell 主迁移完成。
