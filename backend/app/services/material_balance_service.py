...
class MaterialBalanceService:
    ...
    async def run_calculation(
        ...
        try:
            ...
            try:
                ...
                raise Exception(f"Calculation timed out after {timeout_seconds:.1f} seconds. Try reducing simulation time or steps per hour.")
    ...
    def cancel_calculation(self, job_id: str, session: Session) -> bool:
        ...
        if job.status in [MaterialBalanceJobStatus.pending, MaterialBalanceJobStatus.running]:
            job.status = MaterialBalanceJobStatus.cancelled
            job.completed_at = datetime.now()
            job.error_message = "Calculation cancelled by user"
            session.add(job)
            session.commit()
            return True
    ...
    def cleanup_old_jobs(self, session: Session, days_old: int = 30) -> int:
        ...
        statement = select(MaterialBalanceJob).where(
            MaterialBalanceJob.created_at < cutoff_date
        )
