...
class ASM3Service:
    """
    ASM3计算服务：处理ASM3模型的计算任务
    """
    
    def __init__(self):
        self.data_conversion = DataConversionService()
    ...
    async def run_calculation(
        ...
        statement = select(ASM3Job).where(
            ASM3Job.job_id == job_id
        )
        job = session.exec(statement).first()
        
        if not job:
            raise ValueError(f"ASM3 job {job_id} not found")
        ...
        try:
            ...
            print(f"Starting ASM3 calculation for job {job_id}: {total_steps} steps, {num_nodes} nodes, timeout: {timeout_seconds:.1f}s")
            ...
            try:
                ...
                raise Exception(f"ASM3 calculation timed out after {timeout_seconds:.1f} seconds. Try reducing simulation time or steps per hour.")
            
            # Calculate execution time
            calculation_time = time.time() - start_time
            print(f"ASM3 calculation completed for job {job_id} in {calculation_time:.2f} seconds")
            
            # Convert MaterialBalanceResult to dict format for database storage
            print(f"Converting ASM3 result to dict for job {job_id}")
            output_data = {
            ...
            print(f"ASM3 result conversion completed for job {job_id}")
            ...
            print(f"ASM3 calculation cancelled for job {job_id}")
            job.status = MaterialBalanceJobStatus.cancelled
            job.completed_at = datetime.now()
            job.error_message = "ASM3 calculation cancelled due to server shutdown"
            job.result_data = None
            # Re-raise to properly handle the cancellation
            raise
        except Exception as e:
            # Handle calculation error
            error_message = f"ASM3 calculation failed: {str(e)}"
            print(f"ASM3 calculation error for job {job_id}: {error_message}")
    ...
    def _run_calculation_sync(self, input_data: MaterialBalanceInput) -> MaterialBalanceResult:
        ...
        # The MaterialBalanceCalculator already supports ASM3 calculations
    ...
    def cancel_calculation(self, job_id: str, session: Session) -> bool:
        ...
        if job.status in [MaterialBalanceJobStatus.pending, MaterialBalanceJobStatus.running]:
            job.status = MaterialBalanceJobStatus.cancelled
            job.completed_at = datetime.now()
            job.error_message = "ASM3 calculation cancelled by user"
            session.add(job)
            session.commit()
            return True
    ...
    def cleanup_old_jobs(self, session: Session, days_old: int = 30) -> int:
        ...
        statement = select(ASM3Job).where(
            ASM3Job.created_at < cutoff_date
        )
