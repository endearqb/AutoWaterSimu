...
class SimulationInputAdapterError(ValueError):
    """Validation failure while adapting simulation_input.v1 to backend models."""

    def __init__(self, message: str, details: list[dict[str, Any]] | None = None):
        super().__init__(message)
        self.details = details or []

    def to_contract_error(self, trace_id: str = "trace_simulation_input_adapter") -> dict[str, Any]:
        return {
...
def simulation_input_to_material_balance_input(
    ...
    if simulation_input.get("job_type") != "simulation.material_balance.v1":
        raise SimulationInputAdapterError(
            ...
            [_detail("$.job_type", "only simulation.material_balance.v1 is supported", simulation_input.get("job_type"))],
        )
    ...
    try:
        ...
        return MaterialBalanceInput(
            ...
            original_flowchart_data={
                ...
                "customParameters": [{"name": component, "label": component} for component in components],
            },
        )
    except SimulationInputAdapterError:
        raise
...
def _adapt_node(node: dict[str, Any], components: list[str], index: int) -> NodeData:
    ...
    if raw_volume in (None, "") and (is_inlet or is_outlet):
        ...
        raise SimulationInputAdapterError(
            ...
            [_detail(f"$.nodes[{index}].initial_volume", "reactor volume is required", node_id)],
        )
    else:
...
def _adapt_edge(edge: dict[str, Any], components: list[str], index: int) -> EdgeData:
    ...
    for component in components:
        ...
        factor_a.append(_number(factor.get("a"), default=1.0, path=f"$.edges[{index}].concentration_transform.{component}.a"))
        factor_b.append(_number(factor.get("b"), default=0.0, path=f"$.edges[{index}].concentration_transform.{component}.b"))
    ...
    return EdgeData(
        ...
        flow_rate=_number(edge.get("flow_rate"), default=1000.0, path=f"$.edges[{index}].flow_rate"),
    ...
    for index, raw_segment in enumerate(_as_list(value)):
        if not isinstance(raw_segment, dict):
            raise SimulationInputAdapterError(
                ...
                [_detail(f"$.time_segments[{index}]", "time segment must be an object", None)],
            )
...
def _components(simulation_input: dict[str, Any]) -> list[str]:
    ...
    components = [_string_value(component) for component in _as_list(component_schema.get("components"))]
    components = [component for component in components if component]
    if not components:
        raise SimulationInputAdapterError(
            "component_schema.components is required",
            [_detail("$.component_schema.components", "at least one component is required", None)],
        )
    if len(components) != len(set(components)):
        raise SimulationInputAdapterError(
            "component_schema.components must be unique",
            [_detail("$.component_schema.components", "component names must be unique", None)],
        )
    return components
