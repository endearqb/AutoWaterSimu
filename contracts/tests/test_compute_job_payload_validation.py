...
def test_compute_job_fixtures_embed_valid_simulation_input_payload(compute_job_path: Path) -> None:
    compute_job = _load_json(compute_job_path)

    _validate("compute_job.v1.json", compute_job)
    _validate("simulation_input.v1.json", compute_job["payload"])
    assert compute_job["payload"]["schema_version"] == "simulation_input.v1"
