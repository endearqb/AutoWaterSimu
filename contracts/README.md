...
14. `compute_job.v1`、`simulation_input.v1`、`compute_result.v1` 当前支持 `simulation.material_balance.v1`、`simulation.asm1slim.v1`、`simulation.asm1.v1` 与 `simulation.asm3.v1`；`simulation_request.v1` 与 Go API promotion 仍先保留 material balance 边界，避免在未完成 API 路由设计前扩大提交入口。
