...
- UDM Network v2 `takacs_settling.v1` edge transport evaluator tests。
- UDM Network v2 pure RHS assembly tests。
- UDM Network v2 five-model v1 migration parity gate tests。
- UDM Network v2 SecondaryClarifier10Layer reference profile graph tests。
...
| `test_udm_network_transport.py` | UDM Network v2 `takacs_settling.v1` edge transport evaluator tests |
| `test_udm_network_rhs.py` | UDM Network v2 pure RHS assembly tests |
| `test_udm_network_parity_gate.py` | UDM Network v2 five-model v1 migration parity gate tests |
| `test_udm_network_secondary_clarifier.py` | UDM Network v2 SecondaryClarifier10Layer reference profile graph tests |
...
6. `_run_hours` unified reaction dispatch、default no-reaction branch、ASM component contract、ASM oxygen mask scope 或 default clamp 行为改变前，必须同步更新 `test_run_hours_*` / mixed golden correctness-freeze tests、`scripts/audit-simulation-core-correctness-freeze.ps1` 和相关 ADR；默认分支是否 clamp 是显式当前状态，不应在性能 PR 中隐式改变。
