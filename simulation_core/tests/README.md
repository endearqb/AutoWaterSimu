...
6. `_run_hours` mixed-model dispatch、single-model fallback、ASM component contract、ASM oxygen mask scope 或 default clamp 行为改变前，必须同步更新 `test_run_hours_*` / mixed golden correctness-freeze tests、`scripts/audit-simulation-core-correctness-freeze.ps1` 和相关 ADR；默认分支是否 clamp 是显式当前状态，不应在性能 PR 中隐式改变。
