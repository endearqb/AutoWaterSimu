...
- core adapter 行为测试。
- 与 legacy backend 的 material balance、ASM1Slim model-bound 和 `simulation.asm1slim.v1` 数值 parity 测试。
...
4. ASM/UDM 迁移期测试应先覆盖 runtime binding 字段保留和 legacy backend parity，再新增独立 job type 测试；当前仅 `simulation.asm1slim.v1` 已有独立 job type parity。
