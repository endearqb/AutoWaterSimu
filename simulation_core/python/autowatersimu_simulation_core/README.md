...
- UDM Network v2 `network_process_graph.v1` / `network_simulation_input.v1` graph compiler MVP, automatic flow balance solver, passive/reaction node evaluator and `takacs_settling.v1` edge transport evaluator。
...
| `udm_network/` | UDM Network v2 graph compiler MVP, flow balance solver, passive/reaction node evaluator and edge transport evaluator |
