    ...
    try:
        ...
        for node_data in nodes_data:
            node = NodeData(
                ...
                node_type=node_data.get('type') or node_data.get('data', {}).get('node_type', 'default'),
        ...
        for edge_data in edges_data:
            edge = EdgeData(
                ...
                concentration_factor_a=edge_data.get('data', {}).get('factors_a', [1.0]),
                concentration_factor_b=edge_data.get('data', {}).get('factors_b', [0.0])
            )
            edges.append(edge)
...
def create_example_input() -> MaterialBalanceInput:
    ...
    edges = [
        EdgeData(
            edge_id="e1",
            source_node_id="inlet",
            target_node_id="tank1",
            flow_rate=2.0,
            concentration_factor_a=[1.0, 1.0]
        ),
        EdgeData(
            edge_id="e2",
            source_node_id="inlet",
            target_node_id="tank2",
            flow_rate=1.0,
            concentration_factor_a=[0.5, 1.0]
        ),
        EdgeData(
            edge_id="e3",
            source_node_id="tank1",
