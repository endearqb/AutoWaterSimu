...
4. 收紧 `simulation_input.v1` / runtime model 输入字段策略前，先运行 `scripts/audit-simulation-core-input-contract.ps1`；当前 adapter 已有默认 `compat` 与 opt-in `warn` / `strict` 未知字段语义测试，后续重点是 node/edge item schema 与 runtime model `extra` 策略。
