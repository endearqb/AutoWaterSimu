...
- Influent/effluent boundary presets plus UDM Reactor、SecondaryClarifier10Layer、Splitter and Controller node semantics.
...
| `NetworkV2NodeShell.tsx` | Feature-local handles/labels over the allowlisted shared glass node surface and color tokens |
...
2. Do not import legacy Flow node behavior or stores; only the allowlisted glass presentation primitives may be reused.
...
6. Effluent presets remain `boundary` nodes with an inlet port and serialize as `node_type = "sink"`; influent presets serialize as `source`.
