...
export const useUDMStore = create<UDMState>()(
  devtools(
    (set, get) => ({
      // Initial state
      currentJob: null,
      ...
      createCalculationJob: async (input: MaterialBalanceInput) => {
        ...
        try {
          const job = await udmService.createCalculationJob(input)
      ...
      createCalculationJobFromFlowchart: async (flowchartData: any) => {
        ...
        try {
          const job =
            await udmService.createCalculationJobFromFlowchart(flowchartData)
          set({ currentJob: job, isLoading: false })
          return job
        } catch (error) {
          console.error(
            "Error creating UDM calculation job from flowchart:",
            error,
          )
          set({
            error:
      ...
      getCalculationStatus: async (jobId: string) => {
        ...
        try {
          const job = await udmService.getCalculationStatus(jobId)
          set({ currentJob: job, isLoading: false })
          return job
        } catch (error) {
          const errorMessage =
      ...
      getResultSummary: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          const summary = await udmService.getCalculationResultSummary(jobId)
      ...
      getTimeSeriesData: async (params: {
        ...
        set({ isLoading: true, error: null })
        try {
          const data = await udmService.getCalculationTimeseries(params)
      ...
      getFinalValues: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          const data = await udmService.getCalculationFinalValues(jobId)
      ...
      getAnalysisResult: async (jobId: string) => {
        ...
        try {
          const data = await udmService.getAnalysisResult(jobId)
      ...
      validateInput: async (input: MaterialBalanceInput) => {
        set({ isLoading: true, error: null })
        try {
          const result = await udmService.validateCalculationInput(input)
      ...
      getUserJobs: async (skip = 0, limit = 50) => {
        set({ isLoading: true, error: null })
        try {
          const response = await udmService.getUserCalculationJobs(skip, limit)
      ...
      deleteJob: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          await udmService.deleteCalculationJob(jobId)
          const { userJobs } = get()
          set({
      ...
      getJobInputData: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          const data = await udmService.getJobInputData(jobId)
          set({ isLoading: false })
          return data
        } catch (error) {
          const errorMessage =
      ...
      pollJobStatus: (
        ...
        const poll = async () => {
          try {
            ...
            console.error("UDM polling error:", error)
            clearInterval(intervalId)
          }
        }
      ...
      getFlowcharts: async (skip = 0, limit = 50) => {
        set({ isLoading: true, error: null })
        try {
          const response = await udmService.getFlowcharts(skip, limit)
      ...
      createFlowchart: async (flowchart: UDMFlowChartCreate) => {
        set({ isLoading: true, error: null })
        try {
          const newFlowchart = await udmService.createFlowchart(flowchart)
      ...
      getFlowchart: async (id: string) => {
        set({ isLoading: true, error: null })
        try {
          const flowchart = await udmService.getFlowchart(id)
      ...
      updateFlowchart: async (id: string, flowchart: UDMFlowChartUpdate) => {
        set({ isLoading: true, error: null })
        try {
          const updatedFlowchart = await udmService.updateFlowchart(
            id,
            flowchart,
          )
          const { flowcharts } = get()
      ...
      deleteFlowchart: async (id: string) => {
        set({ isLoading: true, error: null })
        try {
          await udmService.deleteFlowchart(id)
...
// 导出类型和状态选择器
export type { UDMState }

// 常用的状态选择器
export const selectCurrentJob = (state: UDMState) => state.currentJob
export const selectResultSummary = (state: UDMState) => state.resultSummary
export const selectTimeSeriesData = (state: UDMState) => state.timeSeriesData
export const selectFinalValues = (state: UDMState) => state.finalValues
export const selectValidationResult = (state: UDMState) =>
  state.validationResult
export const selectUserJobs = (state: UDMState) => state.userJobs
export const selectFlowcharts = (state: UDMState) => state.flowcharts
export const selectCurrentFlowchart = (state: UDMState) =>
  state.currentFlowchart
export const selectIsLoading = (state: UDMState) => state.isLoading
export const selectError = (state: UDMState) => state.error
