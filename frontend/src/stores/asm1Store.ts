...
interface ASM1State
  ...
  > {
  // 继承的属性需要重新声明以确保类型正确
  currentJob: ASM1JobPublic | null
  resultSummary: MaterialBalanceResultSummary | null
  timeSeriesData: MaterialBalanceTimeSeriesResponse | null
  finalValues: any | null
  userJobs: ASM1JobPublic[]
  flowcharts: ASM1FlowChartPublic[]
  currentFlowchart: ASM1FlowChartPublic | null
  isLoading: boolean
  error: string | null
...
export const useASM1Store = create<ASM1State>()(
  devtools(
    (set, get) => ({
      // Initial state
      currentJob: null,
      ...
      createCalculationJob: async (input: MaterialBalanceInput) => {
        ...
        try {
          const job = await asm1Service.createCalculationJob(input)
      ...
      createCalculationJobFromFlowchart: async (flowchartData: any) => {
        ...
        try {
          const job =
            await asm1Service.createCalculationJobFromFlowchart(flowchartData)
          set({ currentJob: job, isLoading: false })
          return job
        } catch (error) {
          console.error(
            "Error creating ASM1 calculation job from flowchart:",
            error,
          )
          set({
            error:
      ...
      getCalculationStatus: async (jobId: string) => {
        ...
        try {
          const job = await asm1Service.getCalculationStatus(jobId)
          set({ currentJob: job, isLoading: false })
          return job
        } catch (error) {
          const errorMessage =
      ...
      getResultSummary: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          const summary = await asm1Service.getCalculationResultSummary(jobId)
      ...
      getTimeSeriesData: async (params: {
        ...
        set({ isLoading: true, error: null })
        try {
          const data = await asm1Service.getCalculationTimeseries(params)
      ...
      getFinalValues: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          const data = await asm1Service.getCalculationFinalValues(jobId)
      ...
      getAnalysisResult: async (jobId: string) => {
        ...
        try {
          const data = await asm1Service.getAnalysisResult(jobId)
      ...
      validateInput: async (input: MaterialBalanceInput) => {
        set({ isLoading: true, error: null })
        try {
          const result = await asm1Service.validateCalculationInput(input)
      ...
      getUserJobs: async (skip = 0, limit = 50) => {
        set({ isLoading: true, error: null })
        try {
          const response = await asm1Service.getUserCalculationJobs(skip, limit)
      ...
      deleteJob: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          await asm1Service.deleteCalculationJob(jobId)
          const { userJobs } = get()
          set({
      ...
      getJobInputData: async (jobId: string) => {
        set({ isLoading: true, error: null })
        try {
          const data = await asm1Service.getJobInputData(jobId)
          set({ isLoading: false })
          return data
        } catch (error) {
          const errorMessage =
      ...
      pollJobStatus: (
        ...
        const poll = async () => {
          try {
            ...
            console.error("ASM1 polling error:", error)
            clearInterval(intervalId)
          }
        }
      ...
      getFlowcharts: async (skip = 0, limit = 50) => {
        set({ isLoading: true, error: null })
        try {
          const response = await asm1Service.getFlowcharts(skip, limit)
      ...
      createFlowchart: async (flowchart: ASM1FlowChartCreate) => {
        set({ isLoading: true, error: null })
        try {
          const newFlowchart = await asm1Service.createFlowchart(flowchart)
      ...
      getFlowchart: async (id: string) => {
        set({ isLoading: true, error: null })
        try {
          const flowchart = await asm1Service.getFlowchart(id)
      ...
      updateFlowchart: async (id: string, flowchart: ASM1FlowChartUpdate) => {
        set({ isLoading: true, error: null })
        try {
          const updatedFlowchart = await asm1Service.updateFlowchart(
            id,
            flowchart,
          )
          const { flowcharts } = get()
      ...
      deleteFlowchart: async (id: string) => {
        set({ isLoading: true, error: null })
        try {
          await asm1Service.deleteFlowchart(id)
...
// 导出类型和状态选择器
export type { ASM1State }

// 常用的状态选择器
export const selectCurrentJob = (state: ASM1State) => state.currentJob
export const selectResultSummary = (state: ASM1State) => state.resultSummary
export const selectTimeSeriesData = (state: ASM1State) => state.timeSeriesData
export const selectFinalValues = (state: ASM1State) => state.finalValues
export const selectValidationResult = (state: ASM1State) =>
  state.validationResult
export const selectUserJobs = (state: ASM1State) => state.userJobs
export const selectFlowcharts = (state: ASM1State) => state.flowcharts
export const selectCurrentFlowchart = (state: ASM1State) =>
  state.currentFlowchart
export const selectIsLoading = (state: ASM1State) => state.isLoading
export const selectError = (state: ASM1State) => state.error
