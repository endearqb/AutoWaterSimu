...
import type { ASM1SlimJobPublic } from "../../../client/types.gen"
...
/**
 * ASM1Slim模型的BubbleMenu组件
 * 使用通用BaseBubbleMenu和ASM1SlimStore
 */
interface ASM1SlimBubbleMenuProps {
  onExport?: () => string
  onImport?: (files: File[]) => void
  onNewFlowChart?: () => void
}

const ASM1SlimBubbleMenu: React.FC<ASM1SlimBubbleMenuProps> = ({
  onExport,
  onImport,
  onNewFlowChart,
}) => {
  ...
  const handleLoadJobData = async (
    ...
    // 获取输入数据和结果数据
    const jobDataResponse = await asm1SlimService.getJobInputData(jobId)
  ...
  return (
    <BaseBubbleMenu
      flowStore={useASM1SlimFlowStore}
      modelStore={useASM1SlimStore}
      onExport={onExport}
      onImport={onImport}
      onNewFlowChart={onNewFlowChart}
      config={{
        enableOnlineSave: true, // ASM1Slim模型支持在线保存
        enableOnlineLoad: true, // ASM1Slim模型支持在线加载
...
export default ASM1SlimBubbleMenu
