...
import type { ASM1JobPublic } from "../../../client/types.gen"
...
/**
 * ASM1模型的BubbleMenu组件
 * 使用通用BaseBubbleMenu和ASM1Store
 */
interface ASM1BubbleMenuProps {
  onExport?: () => string
  onImport?: (files: File[]) => void
  onNewFlowChart?: () => void
}

const ASM1BubbleMenu: React.FC<ASM1BubbleMenuProps> = ({
  onExport,
  onImport,
  onNewFlowChart,
}) => {
  ...
  const handleLoadJobData = async (
    ...
    // 获取输入数据和结果数据
    const jobDataResponse = await asm1Service.getJobInputData(jobId)
  ...
  return (
    <BaseBubbleMenu
      flowStore={useASM1FlowStore}
      modelStore={useASM1Store}
      onExport={onExport}
      onImport={onImport}
      onNewFlowChart={onNewFlowChart}
      config={{
        enableOnlineSave: true, // ASM1模型支持在线保存
        enableOnlineLoad: true, // ASM1模型支持在线加载
