...
interface DefaultNodeProps extends NodeProps<any> {
  store?: () => RFState
}

const DefaultNode = ({ data, selected, id, store }: DefaultNodeProps) => {
  ...
  const [label, setLabel] = useState(
    nodeData.label || nodeData.number || t("flow.node.default"),
  )
  const hoveredNodeId = useHoveredNodeId()
  const isHovered = hoveredNodeId === id
  const inputRef = useRef<HTMLInputElement>(null)
  const tint: GlassTint = "default"
  ...
  useEffect(() => {
    setLabel(nodeData.label || nodeData.number || t("flow.node.default"))
