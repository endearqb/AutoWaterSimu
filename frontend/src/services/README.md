...
- 保留旧 route/component 调用面的 compatibility service facade。
...
| `computeJobsService.ts` | Route-compatible facade that composes `features/*` Compute API wrappers and preserves existing call sites |
...
8. `computeJobsService.ts` 只保留兼容 facade，不继续堆新 endpoint 逻辑；新增 Compute API wrapper 应先放入对应 `features/*/api.ts`，后续再按需补 `features/*/queries.ts`。

## 4. 对外接口

本目录向 stores、routes、components 暴露 legacy typed service functions 和 Compute compatibility facade。
