...
4. `computeApiClient.ts` 和 `computeJobsService.ts` 是 Web bootstrap、route/component 使用 Go Compute API generated client 的边界；`main.tsx`、route/component 不直接 import `frontend/src/client/compute`，需要的 UI-facing 类型由 service 层 re-export。
