...
### 4.0 开发状态回填（2026-03-05）

| 项目 | 当前状态 | 说明 |
|------|----------|------|
| F-2.1 自动均分时段 | ⚠️ 部分完成 | 功能已实现，但 `splitCount` 缺少 `NaN/Infinity` 防护，存在异常风险 |
| F-2.2 时段模板保存/加载 | ⏳ 未开始 | 暂未发现模板导入导出/保存加载实现 |
| F-2.3 时长一致性引导 | ⚠️ 部分完成 | 已显示 simulationHours；错误文案仍未包含期望 hours |
| U-2.1 窄屏可用性 | ⏳ 未开始 | 仍以横向滚动卡片为主 |
| U-2.2 继承值视觉区分 | ✅ 已完成 | 已增加虚线边框 + 图标标识继承态 |
| U-2.3 工具函数去重 | ✅ 已完成 | 已抽取到 `utils/timeSegmentHelpers.ts` |
| U-2.4 排序交互优化 | ⏳ 未开始 | 仍主要依赖上下箭头调整顺序 |
